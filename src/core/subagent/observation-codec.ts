...
import type { SubagentHandleV1, SubagentObservationV1 } from '@/protocol/subagent-provider';
...
export function subagentResultFromObservationV1(
  ...
  const continuation = blockedSnapshot
    ? deserializeSubagentContinuation(blockedSnapshot)
    : undefined;
  ...
  return {
    ...
    >,
    toolRecovery: normalizeToolRecoveryJournalV1(payload.toolRecovery),
    ...(continuation
      ? {
          blocked: {
