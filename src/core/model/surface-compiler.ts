...
import { providerRouteIdentityFromAgentConfigV1 } from '@/core/config/provider-data-admission';
import { computeProviderEndpointIdentityDigest } from '@/core/config/provider-data-policy';
import {
  type AIMessage,
  type BaseMessage,
  isAIMessage,
...
export function compileModelSurfaceV1(input: {
  ...
  const routeIdentity = providerRouteIdentityFromAgentConfigV1(input.config);
  const routeFingerprint = computeProviderEndpointIdentityDigest(routeIdentity) as Sha256DigestV1;
