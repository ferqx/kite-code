...
import { isMcpProviderError } from '@/core/mcp';
...
import type { ProviderReadinessRuntimeRecordV1, RuntimeState } from '@/core/runtime/state';
...
export class ProviderReadinessCoordinatorV1 {
  ...
  private async registerWaiter(
    ...
    const waiterId = digestCapability({
...
function readinessFailure(error: unknown): ClassifiedFailure {
  ...
  if (isMcpProviderError(error)) return classifyMcpProviderError(error);
