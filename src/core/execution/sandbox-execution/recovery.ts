...
import type { SandboxExecutionProviderV1 } from '@/protocol/sandbox-execution-provider';
import type { SandboxExecutionGrantAuthorityV1 } from './grant-authority';
import { sandboxPreparedPlanDigestV1 } from './grant-authority';
import {
  cleanupPosixSandboxRuntimeRootsNoSpawnV1,
  sandboxRuntimeRootsForPreparationV1,
} from './local-runtime-filesystem';
import { reconcilePosixSupervisorV1 } from './posix-supervisor';
import { decodeWindowsRestrictedTokenPreparedTransportV1 } from './windows-preparation';
import { reconcileWindowsRestrictedTokenPreparedV1 } from './windows-runtime';
...
export async function reconcileSandboxPreparationAfterCrashV1(input: {
  ...
  let prepared: Readonly<
    import('@/protocol/sandbox-execution-provider').PreparedSandboxExecutionV1
  >;
  try {
  ...
  if (
    ...
    digestCapability(prepared.backendCapabilities) !== ready.backendCapabilitiesDigest ||
    prepared.enforcement !== ready.enforcement ||
    prepared.resourceSemantics !== ready.resourceSemantics ||
    digestCapability(prepared.cleanup) !== ready.cleanupDigest ||
