...
export class BunSqliteSaver extends BaseCheckpointSaver {
  ...
  setup(): void {
    ...
    this.db.run("UPDATE checkpoints SET created_at = datetime('now') WHERE created_at IS NULL");
  ...
  async getTuple(config: RunnableConfig): Promise<CheckpointTuple | undefined> {
    ...
    return {
      ...
      parentConfig: row.parent_checkpoint_id
        ...
        : undefined,
      pendingWrites: await this.getPendingWrites(row.thread_id, checkpointNs, row.checkpoint_id),
    };
  }

  /** 枚举线程最近 N 个 checkpoint 及首条用户消息摘要 / List recent checkpoints with first user message summary */
  async listCheckpoints(threadId: string, limit: number = 20): Promise<CheckpointEntry[]> {
    this.setup();
    const rows = this.db
    ...
    for (const row of rows) {
      ...
      try {
        const checkpoint = await this.serde.loadsTyped(row.type ?? 'json', row.checkpoint);
        const messages = checkpoint.channel_values?.messages as
          | Array<{ lc_id?: string[]; id?: string[]; content?: unknown }>
          | undefined;
  ...
  async getCheckpointState(
    ...
    if (!tuple?.checkpoint?.channel_values) return null;

    const cv = tuple.checkpoint.channel_values;
    return {
      messages: (cv.messages as import('@langchain/core/messages').BaseMessage[]) ?? [],
      workspaceAccess:
        (cv.workspaceAccess as import('@/core/harness/state').CodeAgentState['workspaceAccess']) ??
        'write',
