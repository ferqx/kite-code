...
/**
 * 双输出流投影：stdout/stderr 逐流截断并各自保留。
 * Dual-stream projection: truncate and preserve stdout and stderr independently.
 *
 * 失败命令的 stdout（如测试输出、部分匹配结果）与成功命令的 stderr 警告
 * 都必须保留——迁移前 Runner 对两路分别截断，投影层承接同一语义。
 * A failing command's stdout (test output, partial matches) and a successful
 * command's stderr warnings must both survive — the pre-migration runner
 * truncated each stream independently, and projection owns the same semantics.
 */
