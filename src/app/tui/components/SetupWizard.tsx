...
const PROVIDER_OPTIONS: ProviderOption[] = [
  { type: 'deepseek', label: 'DeepSeek', defaultBaseURL: 'https://api.deepseek.com/v1', keyHint: 'https://platform.deepseek.com/api_keys' },
  { type: 'openai', label: 'OpenAI', defaultBaseURL: 'https://api.openai.com/v1', keyHint: 'https://platform.openai.com/api-keys' },
  { type: 'openai-compatible', label: 'OpenAI-Compatible', defaultBaseURL: 'http://localhost:8080/v1', keyHint: 'Enter API key for your endpoint' },
...
export default function SetupWizard({ onComplete }: SetupWizardProps) {
  ...
  const baseURLRef = useRef(baseURL); baseURLRef.current = baseURL;
  const selectedRef = useRef(selectedIndex); selectedRef.current = selectedIndex;
  const stepRef = useRef(step); stepRef.current = step;
  const providerTypeRef = useRef(providerType); providerTypeRef.current = providerType;
  const modelIndexRef = useRef(modelIndex); modelIndexRef.current = modelIndex;
  const selModelIndexRef = useRef(selModelIndex); selModelIndexRef.current = selModelIndex;
  const customModelRef = useRef(customModel); customModelRef.current = customModel;
  const thinkingIndexRef = useRef(thinkingIndex); thinkingIndexRef.current = thinkingIndex;
  const selThinkingIndexRef = useRef(selThinkingIndex); selThinkingIndexRef.current = selThinkingIndex;
  const reasoningRef = useRef(reasoning); reasoningRef.current = reasoning;
  const selReasoningRef = useRef(selReasoning); selReasoningRef.current = selReasoning;

  const selectedOption = PROVIDER_OPTIONS.find((o) => o.type === providerType) ?? PROVIDER_OPTIONS[0]!;
  ...
  useEffect(() => {
    ...
    fetch(endpoint, { headers, signal: AbortSignal.timeout(8000) })
      ...
      .then((data: any) => {
        ...
        const names = items.map((it) => (isOllama ? it?.name : it?.id)).filter((n): n is string => typeof n === 'string' && n.length > 0);
        setRemoteModels(names);
      })
      .catch(() => setRemoteModels([]))
      .finally(() => setModelsLoading(false));
  }, [step, remoteModels, modelsLoading, baseURL, providerType, apiKey, selectedOption.defaultBaseURL]);
  ...
  useInput((_input: string, key: { upArrow?: boolean; downArrow?: boolean; return?: boolean; escape?: boolean }) => {
    ...
    if (key.return) {
      ...
      if (opt.type === 'ollama') { setApiKey(''); goToStep('model'); }
  ...
  const submitCredentials = useCallback(() => {
    if (!apiKeyRef.current.trim()) { setError('API key cannot be empty'); return; }
    setError(null);
    goToStep('model');
  }, [goToStep]);
  ...
  useInput((_input: string, key: { escape?: boolean; tab?: boolean }) => {
    ...
    if (key.escape) { setApiKey(''); setBaseURL(''); goToStep('provider'); return; }
    if (key.tab) { setCredFocus((f) => (f === 'apiKey' ? 'baseURL' : 'apiKey')); }
  });

  // ── Step 3: Model ──
  const confirmSetup = useCallback((chosenModel: string) => {
    const name = chosenModel.trim();
    if (!name) { setError('Please enter a model name'); return; }
    setError(null);
    const effort = THINKING_EFFORTS[selThinkingIndexRef.current] ?? 'max';
    const models = remoteModels && remoteModels.length > 0
      ? remoteModels.map((m) => ({ name: m, default: m === name }))
      : [{ name, default: true }];
    const ok = saveProviderConfig({
      name: providerTypeRef.current, type: providerTypeRef.current,
      apiKey: apiKeyRef.current, baseURL: baseURLRef.current || undefined,
      models,
      effort,
      reasoning: selReasoningRef.current,
    });
    if (!ok) { setError('Failed to write config file'); return; }
    onComplete({ modelName: name });
  }, [onComplete, remoteModels]);

  useInput((_input: string, key: { upArrow?: boolean; downArrow?: boolean; return?: boolean; escape?: boolean; tab?: boolean }) => {
    ...
    if (_input === ' ' && !textInputActive) {
      if (modelFocus === 'models' && hasList && mi >= 0) { setSelModelIndex(mi); return; }
      if (modelFocus === 'effort' && ti >= 0) { setSelThinkingIndex(ti); return; }
      if (modelFocus === 'reasoning') { setSelReasoning(reasoningRef.current); return; }
      return;
    }
    if (modelFocus === 'models' && hasList && mi >= 0) {
      if (key.upArrow)   { setModelIndex((s) => Math.max(0, s - 1)); return; }
      if (key.downArrow) { setModelIndex((s) => Math.min(availableModels.length - 1, s + 1)); return; }
    }
    if (modelFocus === 'effort') {
      if (key.upArrow)   { setThinkingIndex((s) => Math.max(0, s - 1)); return; }
      if (key.downArrow) { setThinkingIndex((s) => Math.min(THINKING_EFFORTS.length - 1, s + 1)); return; }
    }
    if (modelFocus === 'reasoning') {
      if (key.upArrow || key.downArrow) { setReasoning((r) => !r); return; }
    }
  });
  ...
  const itemColor = (active: boolean, sel: boolean) =>
    active || sel ? t.primary : t.muted;
    ...
    return (
      <Box flexDirection="column" padding={1}>
        <Box flexDirection="column" borderStyle="round" borderColor={t.primary} paddingX={2} paddingY={1}>
          ...
          {error ? (
            <Box><Text color={t.error}>{error}</Text></Box>
          ) : null}
          <Box marginTop={1} flexDirection="column">
            <Box>
              <Text color={focusColor(credFocus === 'apiKey')}>
                {cur(credFocus === 'apiKey')} API Key   </Text>
              <TextInput
                value={apiKey}
                onChange={(v) => { setApiKey(v); apiKeyRef.current = v; }}
            ...
            <Box>
              <Text color={focusColor(credFocus === 'baseURL')}>
                {cur(credFocus === 'baseURL')} Base URL  </Text>
              <TextInput
                value={baseURL}
                onChange={(v) => { setBaseURL(v); baseURLRef.current = v; }}
  ...
  if (step === 'model') {
    ...
    const activeModelName = hasList && selModelIndex >= 0 ? selModelName : (customModel || selModelName || '');
    ...
    return (
      <Box flexDirection="column" padding={1}>
        <Box flexDirection="column" borderStyle="round" borderColor={t.primary} paddingX={2} paddingY={1}>
          ...
          {/* Model */}
          <Box marginTop={1}>
            <Text bold color={focusColor(modelFocus === 'models')}>Model</Text>
          </Box>
          {modelsLoading ? (
            <Box paddingLeft={2}><Text color={t.dim}>Fetching…</Text></Box>
          ) : hasList ? (
            ...
            <Box paddingLeft={2}>
              ...
              <TextInput
                value={customModel}
                onChange={(v) => { setCustomModel(v); customModelRef.current = v; }}
          ...
          {/* Reasoning */}
          <Box marginTop={1}>
            <Text bold color={focusColor(modelFocus === 'reasoning')}>Reasoning</Text>
          </Box>
          <Box flexDirection="column" paddingLeft={2}>
            <Item
          ...
          {selReasoning && (
            <>
              <Box marginTop={1}>
                <Text bold color={focusColor(modelFocus === 'effort')}>Effort</Text>
              </Box>
              <Box flexDirection="column" paddingLeft={2}>
