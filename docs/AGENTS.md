...
9. 可写任务必须位于独立 branch/worktree，且只有一个 Git owner。协作 Agent 可以读取、测试或修改明确分配的互斥路径，但不得 stage 或提交。
10. 两个任务命中同一 workspace/current authority 时必须串行，不得通过共享 dirty worktree 或临时兼容副本并发修改同一事实。
