...
- Supervisor 的 `start()`/`stop()` 幂等；App 请求的 reload、retry 和 Repository mutation 进入同一串行 reconcile 队列并再次经过项目审批门禁；
...
Runtime diagnostic 只表达 code、retryable、脱敏 message 和有限 technical fields。Runtime 不提供展示标题或操作文案。URL 只保留 origin，authorization、token、secret 和 query credential 必须脱敏；TUI 决定标题、颜色、截断和建议动作。
