...
启动会话索引只为每个候选会话解码到第一条可用于命名的用户消息，找到后立即停止；不得为了构造 `/resume` 列表而在 TUI 主线程解码每个会话的完整事件日志。打开具体会话时仍由 session-scoped Store 完成全量 current-format、snapshot 与恢复边界校验，列表优化不放宽恢复完整性。
