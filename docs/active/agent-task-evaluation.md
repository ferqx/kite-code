...
projection 的宿主路径分隔符。closure 对 repository text 以 UTF-8/LF 规范字节摘要，不能把 Windows `autocrlf`
materialization 当作不同 qualification。任何 closure 或 qualification wrapper drift 都必须同一改动重算 manifest digest
