...
namespace、只读绑定必需系统根，并把 checkout 与复制到 owner-only staging root 的 Bun executable 分别投影到固定隔离根；只给
独立 owner-only private runtime bind 写权限，Bun cache、HOME 与临时文件都必须落入该私有根，不得依赖宿主 runner 的安装路径、
checkout 祖先目录或默认 cache；在任何仓库代码运行前，系统 `setpriv` 再降回 runner UID/GID、清空 groups/capability、建立
