...
父 Runtime 与 Subagent descendant 的 budget admission 使用同一个 terminal adapter。普通Tool/Shell不再因
活动数量进入waiter；累计额度不足为 `budget_exhausted`，已有 unknown invocation 为
