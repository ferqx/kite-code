...
验证：`bun test tests/runtime/failures.test.ts tests/runtime/deadline.test.ts tests/runtime/scheduler.test.ts tests/shell-exec.test.ts`。
...
- `cancellation_cleanup_failed`：无法确认受控进程树收敛；terminal result 保留该 code，
  Scheduler 必须进入 recovery-blocked，直到外部完成受控进程树 reconciliation。
