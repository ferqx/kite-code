...
- `task` 的 raw 模型/legacy 形态是严格闭合的 `{subagent_type, task}`；private `taskArtifact` 形态是独立的严格闭合分支。二者不得混合，否则 Registry 与 Tool Pipeline 必须在 hydration、Provider 与 child dispatch 前返回 `invalid_arguments`。这只收紧未知字段边界，不改变 legacy v24 已持久化 raw Task 的只读 restore，也不把 private 字段暴露到模型 schema。
