...
M2 使用 `StructuredContextSummaryV2`（向后兼容 V1）。V2 新增 `coveredUserMessageIds` 强制覆盖校验、分证据 `evidenceMessageIds` 追踪、`completedEffects` 替代 `completedWork`（用 `effectClass` 判定而非 `path` 存在性）。Runtime 先从任务目标、用户请求、文件与 side-effecting capability 结果、失败和最近可靠观察生成 deterministic fact ledger；ledger 分别保留 canonical text、operation、outcome、error、consequence 与不可变资源元数据，summary model 不能改写这些语义事实或决定 mandatory fact 是否保留。用户消息的 source fact 固定为 `user_request`，但 summary item 可用 `sourceFactIds` 把它分类到 `userConstraints` 或 `decisions`；mandatory coverage 对分类项只计算 `sourceFactIds`（缺省时才使用 `factId`），validator 逐一验证原 source fact 的 canonical text 与 evidence，因此分类 ID 不能借用其他 mandatory fact。Observation 的 `keyFacts` 必须与 ledger canonical text 精确一致。候选结果依次经过 JSON/Zod、完整 provenance 集合精确匹配、fact 语义与 evidence binding、mandatory fact IDs、user message coverage 和基于同一次 effect、同一 projection 输入及当前模型 preflight 计算的 before/after token gain 与 target 校验，并只允许一次 schema repair。M2 的可靠 observation 复用 `ResourceObservationTracker`：截断、legacy digest 或缺少 raw digest/revision 的 projected digest 不构成可靠观察，path write、未知范围 write 和 workspace mutation 会使旧观察失效。超出单次输入预算或 hard 路径校验失败时按完整 turn/tool block 分块；chunk summary 只是 merge 输入，只有最终 summary 能成为 checkpoint。

启用 `contextCompactionAutoV1` 后，Model Controller 在 provider 调用前通过统一的 `buildContextProjection()` 入口计算五级 context pressure（`normal` / `warning` / `compact_due` / `hard_limit` / `unknown`），取代旧的三级 soft/hard 判断。正常模型调用、compaction effect、`/context` 与 `/compact reset` 先通过同一个 `resolveContextProjectionEnvironment()` 重建当前 built-in tools、已绑定 MCP tools、tool search、workflow skill descriptors 与 active inline skill instructions，并共享包含 adapter metadata 的 resolved model capabilities；before/after 必须共享该环境，正式 acceptance 不读取旧 preflight 的 estimate。`compact_due`（默认利用率 88%）触发 proactive M2 压缩，受 cooldown、收益预检、thrash breaker（`AutoCompactionGuard`）约束；只有 `auto_soft` / `auto_hard` 的低收益失败累计 breaker，成功的 `manual` / `manual_recovery` 明确解除 breaker。`hard_limit`（默认 94%）在无 safe boundary 时产生 durable `hardBlock`，持久阻断普通模型调用；此时 scheduler 只放行显式 `manual_recovery` compaction，且仅在恢复压缩成功后清除 block。已有 checkpoint 时执行增量压缩（baseSummary + 新 tail 结构化 merge）；模型生成的自由文本 `unresolvedQuestions` 不得进入 durable checkpoint，旧 checkpoint 恢复时也会丢弃该字段。`ProviderContextOverflowError` 标准化 overflow 检测，每 turn 只允许一次 recovery，重复 overflow 产生 durable hardBlock。Active checkpoint 只替换模型投影中的 covered prefix，原始 transcript 继续由 RuntimeStore 保存。Context preflight、threshold 与 target 共同使用配置的 provider safety ratio。Compaction lifecycle、context pressure、token savings 与真实 effect duration 写入 session trace，不能只依赖进程内指标。
