...
M2 使用 `StructuredContextSummaryV2`（向后兼容 V1）。V2 新增 `coveredUserMessageIds` 强制覆盖校验、分证据 `evidenceMessageIds` 追踪、`completedEffects` 替代 `completedWork`（用 `effectClass` 判定而非 `path` 存在性）。Runtime 先从任务目标、用户请求、文件与 side-effecting capability 结果、失败和最近可靠观察生成 deterministic fact ledger；ledger 分别保留 canonical text、operation、outcome、error、consequence 与不可变资源元数据，summary model 不能改写这些语义事实或决定 mandatory fact 是否保留。用户消息的 source fact 固定为 `user_request`，但 summary item 可用 `sourceFactIds` 把它分类到 `userConstraints` 或 `decisions`；validator 仍逐一验证原 source fact 的 canonical text、evidence 和 mandatory coverage，因此分类变化不授予改写事实的权限。候选结果依次经过 JSON/Zod、完整 provenance 集合精确匹配、fact 语义与 evidence binding、mandatory fact IDs、user message coverage 和基于同一次 effect、同一 projection 输入计算的 before/after token gain 校验，并只允许一次 schema repair。M2 的可靠 observation 复用 `ResourceObservationTracker`：截断或 legacy digest 不构成可靠观察，path write、未知范围 write 和 workspace mutation 会使旧观察失效。超出单次输入预算或 hard 路径校验失败时按完整 turn/tool block 分块；chunk summary 只是 merge 输入，只有最终 summary 能成为 checkpoint。
