...
`check:runtime-packages` 是 RM-16 Required Gate；RM-16 与 RM 总计划已经 completed，完成证据绑定
...
RA 已在真实持久化、序列化或进程外边界完成 identity/integrity/format production cutover；同进程 Kernel/Host/Builtin
继续是可信 typed seam，详见 `runtime-authority-boundary.md`。本文中标注为 RM-xx 的段落记录当时的分阶段不变量；当前 format 事实统一为 Runtime State、SQLite Store 与 `kite-runtime-modularization-v1-2026-08-19`，不得把历史“保持 Runtime State/SQLite Store”语句解释为现行 composition。
...
RM 当前 authority seam 必须与“operation owner”分开阅读：Agent Kernel 只产生纯 effect；App 只组合一个
`ModelInvocationGateway` 与一个 `BuiltinModelOperationExecutionPort`；Host 只提供同一 frozen SPI snapshot
对应的 `CapabilityExecutionPort`、generic lifecycle/lease/transaction/abort/notification；Builtin catalog
snapshot 才是 schema/parser/effects/traits/operation identity authority。当前 Runtime State 的 App orchestration 只通过
...
机制，Kernel 负责纯 Runtime State decision/reducer。旧 Core tree 与 bootstrap legacy tree 已无 production source。
dynamic MCP 保持独立 binding/descriptor/catalogRevision route；`ask_user` 仍由 Kernel request-user-input interrupt terminal
拥有。RM-16 的 caller/owner closure、Required Gate 与完成证据已经闭合；RA 不得反向恢复第二 owner。

### 1.1 RM-15 Model/Builtin、Runtime Registry、Pure Kernel、Host lifecycle、Client Contract 与 Storage 状态

RM 已建立六个私有 workspace package 和目标 App：
...
RM-03 已建立 App 可见的 `RuntimeAccess.command/query/subscribe`、Command Receipt、Session/Work/Turn/
...
`createKiteRuntimeBoundary`、`runCli` 和 `runTui`，过渡 `src/index.ts` 已物理删除，不再导出或构造 Core Runtime
authority。
...
RM-06 的幂等 receipt 与 projection history 仍只覆盖当前 Host 生命周期，因为 SQLite Store 没有 durable command ledger
...
拥有；App bridge 只执行已准备的单一路径并把 current Runtime State facts 投影回 Host。

RM-04 已把 production persistence API 迁到 `@kite/runtime-host/storage` port 与
...
与 `packages/runtime-storage-sqlite/src/` 是唯一 SQLite Store storage seam。`apps/kite/src/bootstrap.ts` 是唯一
...
root tests 只经 Host Runtime State codec 与 SQLite Store adapter 组合同一 storage seam，
不复制 concrete driver。

adapter 将 decision、attempt-start、receipt/evidence、terminal/recovery 四类 transaction port 精确映射到
当前单次 event+snapshot 原子事务。RM-06 的 Host `EffectSupervisor` 在这些 port 前提供唯一 acknowledgement
owner，并把当前 effect lease owner/expiry 作为 terminal transaction 的原子前置条件；lease claim/renew/release、
checkpoint/fork/rewind 保持 SQLite Store 语义。执行者取得的 owner token 通过 execution context 与 Kernel batch 贯穿到
...
RM-10 至 RM-15 迁入 Builtin；Kernel decision authority 已在 RM-07 切到纯包。
...
非 bootstrap App 对 Host/Registry/Builtin catalog/SQLite authority factory 的导入与 RA format 泄漏。App
子组合仍可消费显式 export 的 SPI type、Builtin presentation/config/mechanism 与 Host observability contract；这些
导入不构造第二 Host、Store、Registry 或 frozen catalog。Runtime State、SQLite Store、epoch
...
RM-07 将 production transition owner 切到 `@kite/agent-kernel` 的
...
Host、SPI、App，也不执行 effect。当前 Runtime State 的 event normalization、domain reducer、invariant 与 scheduler 通过
固定 `KernelDomain` 在 composition 时绑定；这只是 RM-16 物理 domain split 前的编译期适配，不是第二 transition
owner。生产 `AgentKernel.processEvent/processEventBatch` 先取得纯 decision，再通过唯一 SQLite Store port 原子提交
event/snapshot，提交成功后才推进进程内 State。

Host 仅在 applied receipt 与 committed revision 已形成后签发 RM 最小 `AuthorizedEffect`，并把它交给
...
operation registration 与 production caller closure 均已清零，剩余只待 RM-16 Required Gate 证据闭合。
`RuntimeKernelControl` 与 `createAgentKernel` 旧 production surface 已删除；
Runtime State restore/recovery 由 `RuntimeSessionCoordinator` 与 Host Runtime State session seam 负责；不存在旧 Kernel coordinator。

RM-08 把 module lifecycle 与 production registry 切到 `@kite/runtime-spi` / `@kite/runtime-host`。Registry
...
Receipt/Grant/Context DTO 仍是私有进程内 transport；当前持久边界使用 strict Runtime State/SQLite Store 与 checksum，
...
RM-09 将 Runtime State 的精确 turn-scoped `CapabilityBinding` DTO 冻结在 `@kite/runtime-spi`，并把唯一 binding
构造者切到 `@kite/builtin-runtime#createCapabilityBinding`；`@kite/runtime-spi` 的 immutable registry snapshot 与
`arbitrateCapability` 只解析 binding、definition、provider、executor 与 revision/schema 一致性，返回 typed
...
具体工具的调度元数据由冻结 Builtin catalog declaration 声明，Runtime State 调度前才投影为 resource scopes、access、conflict keys、
...
RM-10 建立 Host-owned `CapabilityExecutionPort`：它只对启动时冻结的 Registry 做 exact arbitration、
...
JSON facts，SQLite Store 原子确认 invocation intent 与 attempt 后，唯一 Builtin executor 才可读取这些 facts 并返回
SPI Receipt。Receipt 继续经 Host Tool Pipeline Artifact/terminal commit 到 Kernel 和 Client，Legacy operation 与
旧 central executor 已物理删除。没有 second executor、fallback 或双写；Runtime State、SQLite Store 和 epoch 不变。

RM-11 又把 Skills、Context compiler/source、MCP/Web 的 8 个 operation 切给 Builtin Runtime；RM-12
...
RM-13 又把 `shell_execute` 的 definition/executor 切给 `@kite/builtin-runtime`，把 Sandbox Provider contract
...
已在 RM-16 物理删除。RM-13 checkpoint 的四个 Builtin module 合计拥有 16 个
operation、Legacy module 在该历史 checkpoint 尚余 13 个；当前 Legacy module 已清零，RA authority/identity/single-Host/format 已闭合。

RM-14 把 8 个 interaction/planning/Subagent/Verification operation、Subagent/Verification private SPI、
deterministic verification executor、Local Provider/composition、continuation/role ceiling/replay semantics 与
child lifecycle registration/single-use/expiry owner 切给 `@kite/builtin-runtime`。在 RM-14 checkpoint，旧树尚保留
Runtime State adapter、Plan/Artifact store 与待迁移的 Model runner/message adapter；这些 compatibility owners 已在 RM-16 删除，Verification Policy/Completion 始终由
Kernel 唯一裁决。该 checkpoint 的五个 Builtin module 合计拥有 24 个 operation，Legacy module 只剩 5 个 Model purpose；
Runtime State、SQLite Store、原 epoch 与安全行为未改变。

RM-15 已把 Model Surface 私有 contract 迁到 `@kite/runtime-spi/model`，把 Gateway、transport、response source、
...
`BuiltinModelEffectCoordinator`。auto-review、Verification reviewer、context compactor 与 primary/subagent model step
...
`runtime-tool-effect.ts` 与 `turn-coordinator.ts` 是唯一 Runtime State effect orchestration seam；Host generic
...
Core/App production caller 与第二 compaction coordinator 均已清零。该源码 closure 尚需 RM-16 的最终 manifest、docs、
...
Runtime State/SQLite Store session seam；idle `/compact` 复用同一 Kernel、Store、Builtin model composition/Gateway 与 Host capability
...
当前 Runtime State、SQLite Store、epoch `kite-runtime-modularization-v1-2026-08-19`、canonical Workspace identity、
...
| 概念           | 当前目录                                                                                                                                                                                                                                                                   | 核心实现                                                                                                                                                                                             | 架构职责                                                                                                                                                                                                    |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Agent          | `apps/kite/src/bootstrap/runtime/RuntimeSessionCoordinator.ts`、`turn-coordinator.ts`、`packages/builtin-runtime/src/model/`                                                                                                                                               | `executeRuntimeTurn()`、Builtin Model Gateway/Context compiler                                                                                                                                        | 结合 Runtime 投影调用模型，产出工具调用或最终回答；App coordinator 注入 Kernel session、effect port、concrete Model 与 composition，Builtin 拥有 Model/Prompt 语义 |
| Runtime Kernel | `packages/agent-kernel/src/`；Runtime State reducer/domain 位于该 package；持久化 port 位于 `packages/runtime-host/src/storage.ts`                                                                                                                                                  | `decide()`、`reduceAgentState()`、`selectPendingEffects()`、`selectSchedulableEffectBatch()`、`KernelInput`、`DecisionFacts`                                                                                 | 唯一纯状态转换与调度权威；只消费显式 State/Input/Facts/ExecutionTraits/domain，不读外部 I/O；Host 负责翻译与 SQLite Store commit |
| Capability     | `packages/runtime-spi/src/`、`packages/builtin-runtime/src/`、`apps/kite/src/bootstrap/runtime/tool-pipeline-*.ts`                                                                                                                                                          | `CapabilityDefinition`、`CapabilityBinding`、`CapabilityRegistrySnapshot`、`arbitrateCapability()`、`createCapabilityBinding()`、Builtin operation modules                                 | 分离 Catalog/Disclosure/Binding/Proposal/Intent/Grant/Receipt；使用稳定 ID、不可变 revision 和轮次绑定，arbitration 不授权或执行；具体 Filesystem/Git/MCP/Skill/Web/Model 语义由 Builtin 拥有 |
| Policy         | `packages/agent-kernel/src/authorization.ts`、`apps/kite/src/bootstrap/runtime/tool-policy.ts`、`packages/builtin-runtime/src/sandbox/`                                                                                                                                    | Runtime State governance facts、App policy adapter、Builtin sandbox policy facts                                                                                                                               | 对有效副作用进行分类，执行模式限制、授权、审批和技术隔离；Builtin Sandbox 不自行授权 |
| Execution      | `packages/builtin-runtime/src/planning-operations.ts`、`subagent-operations.ts`、`verification-operations.ts`、`packages/builtin-runtime/src/model/`、`packages/runtime-host/src/process-spawn.ts`、`packages/runtime-host/src/process-output.ts`、`apps/kite/src/bootstrap/runtime/runtime-effect-coordinator.ts`、`runtime-tool-effect.ts` | `createPlanningRuntimeModule()`、`createSubagentRuntimeModule()`、`createVerificationRuntimeModule()`、`ModelInvocationGateway`、Host process supervisor、`createAppRuntimeEffectExecutor()`、`executeAppRuntimeToolsEffect()` | Builtin 执行已获准的领域 operation，Host 唯一监督通用机制，App 显式组合 concrete mechanism；App effect coordinator/turn coordinator 是唯一 Runtime State orchestration seam |
| Verification   | `packages/runtime-spi/src/verification.ts`、`packages/builtin-runtime/src/verification/`、`apps/kite/src/bootstrap/runtime/verification-effect.ts`                                                                                                                                 | `VerificationSpec`、`executeDeterministicVerificationChecks()`、`executeVerificationEffect()`、`resolveVerificationMode()`                                                                       | Builtin 依据 Receipt、Artifact 和注入 port 形成 check evidence；Kernel/Runtime 唯一决定通过、修复、重规划、补偿或 waiver |
...
Model Surface V1 已完成 MS-01–MS-04 migration series；RM-15 后由
...
storage 与 `ModelArtifactStore` 以 keyed opaque ref 保存严格 schema 的 Surface/Response/Provider options。
Agent、compaction、auto review、verification review 与 subagent step 都先编译同一冻结 Surface，再通过唯一
`ModelInvocationGateway` 调用 single-attempt transport；旧 `invokeBoundModel` 权威已删除。Gateway 在每次
...
`ModelResponseSource`，所有 response/attempt identity 都由 Builtin Gateway、Host ack 与 Runtime State persistence seam
...
Host 翻译 Command / Receipt / Host fact → KernelInput + DecisionFacts
  ...
  → Host SQLite Store port 原子提交 event / snapshot
  → 提交成功后推进进程内 State
...
├── state.ts       Runtime State 及 capability/skill/verification 投影
├── events.ts      已发生的事实
...
├── RuntimeSessionCoordinator.ts  App 唯一 Runtime State session/coordinator
...
CUT-01/RA 已把 Production Runtime 切换到 schema v26 与 format epoch
...
`ToolOutcome`、Tool recovery journal 与 low-information Subagent lifecycle；restore 不执行 historical event
...
当前 schema 还持久化 parent/subagent 共用语义的 `ToolRecoveryJournal`。journal 的确定性
...
`decideAutoReview` 从 reviewer facts 决定，Runtime State adapter 使用 `escalatedToUser` 保持非终态并转人工审批；
...
Runtime State session recovery 的严格校验会误判健康 snapshot。恢复时父与所有 suspended child 属于同一 identity
...
`boundedCancellation` 使用 budget deadline 驱动统一 AbortSignal。取消事务先 release
...
`RuntimeSchedulingPolicy` 从实际 scheduler 常量导出 parallel-read allowlist/ceiling/barrier、
shell overlap/approval/rejection、FIFO compound admission 和 late-event policy 的唯一 canonical
snapshot/digest。Release tooling 只能 hash/消费该 snapshot。默认关闭的 `resourceBudget`
不能单独生成 production 资格。

Task 1C.5 的 `resolveFailureMode()` 将 RFC failure matrix 固化为 `@kite/agent-kernel` 的封闭 policy table。它在不
...
和 run deadline producer 已直接接线；suite 将全部 terminal resolution 通过 Host Runtime State snapshot
...
手动 `/compact` 同样不能绕过 Kernel。Host recovery 后，App shell 对空闲 session 复用 `RuntimeSessionCoordinator` 的 Runtime State/SQLite Store session seam 执行单次 `compact_context`；若 agent loop 正在运行，则使用同一 `runtime-effect-coordinator.ts` 暴露的受限 live control 只注入 RuntimeEvent，依靠现有 scheduler 排队。没有 Host-recovered session 时 fail closed；context/status/planning 的兼容投影同样复用同一 App coordinator，不打开 standalone Kernel。Live control 不暴露可变 State 或直接 reducer，外部事件推进 revision 后，正在运行的旧 effect 仍由 lease 机制判 stale。

TUI Plan Mode 切换遵循相同的 writer 边界：进入与退出事件只提交给 `RuntimeSessionCoordinator` 的 Runtime State session，并通过 batch 保持 placeholder 创建或 planning 取消的原子性；空闲 session 也必须通过 Host-recovered coordinator，缺失时 fail closed。App 不得为同一 thread 创建第二 Kernel writer，否则 SQLite Store CAS 虽会阻止 stale snapshot 覆盖，但会把正常的 Plan 切换退化为 revision conflict 并终止当前 run。
...
reconciliation 副作用；RuntimeSessionCoordinator/Host Runtime State recovery seam 把未终结 invocation、reservation
和 waiter 收敛为可审计的 unknown/released/cancelled 事实。这样 Session 列表读取不会因观察历史
会话而改写 Runtime Store，真正恢复执行时仍保持保守收敛。

Runtime fault/soak 仍是 RM-16 的独立最终 Gate；本页只定义其必须绑定当前源码、Runtime State/SQLite Store receipt provenance 与完整 Runtime ledger 的证据边界，不把任何本地输出或历史报告当作完成证据。
...
算法生成与 Runtime State 完全相同的 `bindingId/schemaDigest`；执行前必须重新核对 binding token、turn、capability
...
Tool Pipeline；child terminal durable 提交后才交回 `BuiltinChildRuntimeDriver`。PS-03 当前已把唯一生产
`LocalSubagentProvider` 接到 normal task、approval resume 与 Skill fork，并以 exact sealed grant 绑定父 attempt、
child identity、role/task、ceiling/binding、authorization/mode、workspace boundary、budget/cancel 与 Model replay
authority。每次 child model step 通过 App 注入的唯一 `BuiltinModelEffectCoordinator` 执行；Builtin 从同一 frozen
...
App Runtime State tool/receipt adapter 之间只有 invocation-scoped callback，且无生产 fallback。private
...
PS-01 已把五个 Workspace 文件工具接到 `WorkspaceFilesystemProvider`。Tool Pipeline 只在
...
`commitMutation`。`LocalWorkspaceFilesystemProvider` 及其 descriptor-relative native helper 是生产路径
...
PS-02 将 Sandbox 的 execution backend 抽成 protocol-first `SandboxExecutionProvider`：Pipeline/Kernel 保留
...
cleanup。Builtin 的 `createPlanningRuntimeModule()` 是 `builtin:shell_execute` 唯一 operation owner，Builtin catalog entry
...
默认关闭的 `mcpProviderAction` 只增加 Runtime lifecycle，不把 control-plane mutation 移入 Kernel/Host。Runtime 持久化 required/started/completed/deferred/failed，App shell 执行 login/approve/retry；成功后强制新 turn，defer/failure 则留下明确事实。TUI 把 required 事件投影到既有 foreground/background interrupt surface，并由 App controller 委托 Supervisor。
...
VerificationExecutor --> VerificationSpec
...
当前架构只保留真实边界：Project identity 是 canonical Workspace 的确定性标识；没有持久 Store/Handle。
SQLite Store 使用 strict canonical Event 与 snapshot checksum；child process 使用 OS channel/process identity 与
`RuntimeControlFrame`。没有 installation key、HMAC/authenticator、DataOrigin/EgressAuthority 或 global Host lease。
真实外部 Credential 仍按 provider/profile/purpose 由共享 broker 管理。

SQLite Store/Runtime State 是 Runtime Session 的唯一 production path；不保留旧格式 constructor、双写或 fallback。
...
production owner 是 App bootstrap + Runtime Host SQLite adapter：bootstrap 只创建
`.runtime-state-store.db` target path，Runtime State codec 在 storage boundary 处恢复为 Kernel 可消费的 typed
state；任何 format epoch/session mismatch 都 fail closed，Runtime 不创建内部 key。

Named snapshot/fork/rewind/delete 复用同一 Runtime State/SQLite Store owner；fork 重绑 Session/recovery identity，
...
> 路径同步：当前实现中的 State/Store 文件已采用无版本文件名（例如 `state-runner.ts`、`state-storage.ts`、`store.ts`）；Runtime State/SQLite Store 仅表示持久格式 metadata，不表示源码实体名称。
...
> SQLite boundary cleanup: `preflight.ts` and `adapter.ts` implement one current path; old sidecar helpers and compatibility tests are deleted.
> Session ownership split: `runtime/session/runtime-session.ts` owns one live session, `session-manager.ts` owns registry/lifecycle orchestration, and `session-registry.ts` owns lookup and active-session identity.
...
> Subagent API synchronization: role/config helpers are available through `@kite/builtin-runtime/subagent` for domain consumers; the root barrel remains reserved for composition surfaces.
> TUI adapter boundary: `adapters/tui/runtime-bridge.ts` projects the neutral Runtime Session contract; Runtime Session does not import TUI types.
