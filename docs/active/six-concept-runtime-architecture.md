...
M2 使用 `StructuredContextSummaryV2`（向后兼容 V1）。V2 新增 `coveredUserMessageIds` 强制覆盖校验、分证据 `evidenceMessageIds` 追踪、`completedEffects` 替代 `completedWork`（用 `effectClass` 判定而非 `path` 存在性）。Runtime 先从任务目标、明确用户约束、文件与 side-effecting capability 结果、失败、verification、plan 引用和最近可靠观察生成 deterministic fact ledger；summary model 只能压缩叙述，不能决定 mandatory fact 是否保留。候选结果依次经过 JSON/Zod、provenance、mandatory fact IDs、user message coverage 和基于候选投影的 token gain 校验，并只允许一次 schema repair。超出单次输入预算或 hard 路径校验失败时按完整 turn/tool block 分块；chunk summary 只是 merge 输入，只有最终 summary 能成为 checkpoint。

启用 `contextCompactionAutoV1` 后，Model Controller 在 provider 调用前通过统一的 `buildContextProjection()` 入口计算五级 context pressure（`normal` / `warning` / `compact_due` / `hard_limit` / `unknown`），取代旧的三级 soft/hard 判断。`compact_due`（默认利用率 88%）触发 proactive M2 压缩，受 cooldown、收益预检、thrash breaker（`AutoCompactionGuard`）约束。`hard_limit`（默认 94%）在无 safe boundary 时产生 durable `hardBlock`，持久阻断模型调用直到显式 recovery。压缩结果校验使用候选 checkpoint 投影获取真实 after estimate（非近似减法）。已有 checkpoint 时执行增量压缩（baseSummary + 新 tail 结构化 merge）。`ProviderContextOverflowError` 标准化 overflow 检测，每 turn 只允许一次 recovery，重复 overflow 产生 durable hardBlock。Active checkpoint 只替换模型投影中的 covered prefix，原始 transcript 继续由 RuntimeStore 保存。
