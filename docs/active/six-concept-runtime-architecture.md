...
启用 `contextCompactionAutoV1` 后，Model Controller 在 provider 调用前通过统一的 `buildContextProjection()` 入口计算五级 context pressure（`normal` / `warning` / `compact_due` / `hard_limit` / `unknown`），取代旧的三级 soft/hard 判断。正常模型调用、compaction effect、`/context` 与 `/compact reset` 先通过同一个 `resolveContextProjectionEnvironment()` 重建当前 built-in tools、已绑定 MCP tools、tool search、workflow skill descriptors 与 active inline skill instructions；before/after 必须共享该环境，正式 acceptance 不读取旧 preflight 的 estimate。`compact_due`（默认利用率 88%）触发 proactive M2 压缩，受 cooldown、收益预检、thrash breaker（`AutoCompactionGuard`）约束；只有 `auto_soft` / `auto_hard` 成功记录进入自动 thrash 历史。`hard_limit`（默认 94%）在无 safe boundary 时产生 durable `hardBlock`，持久阻断普通模型调用；此时 scheduler 只放行显式 `manual_recovery` compaction，且仅在恢复压缩成功后清除 block。已有 checkpoint 时执行增量压缩（baseSummary + 新 tail 结构化 merge）。`ProviderContextOverflowError` 标准化 overflow 检测，每 turn 只允许一次 recovery，重复 overflow 产生 durable hardBlock。Active checkpoint 只替换模型投影中的 covered prefix，原始 transcript 继续由 RuntimeStore 保存。Context preflight、threshold 与 target 共同使用配置的 provider safety ratio。
