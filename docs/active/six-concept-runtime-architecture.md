...
相关：ADR-0001、ADR-0007、ADR-0008、ADR-0021、ADR-0022、ADR-0024、`mcp-runtime-governance.md`、`verification-governance.md`、`capability-progressive-disclosure.md`。
...
M2 使用 `StructuredContextSummaryV2`（向后兼容 V1）。V2 新增 `coveredUserMessageIds` 强制覆盖校验、分证据 `evidenceMessageIds` 追踪、`completedEffects` 替代 `completedWork`（用 `effectClass` 判定而非 `path` 存在性）。Runtime 先从任务目标、用户请求、文件与 side-effecting capability 结果、失败和最近可靠观察生成 deterministic fact ledger；ledger 分别保留 canonical text、operation、outcome、error、consequence 与不可变资源元数据，summary model 不能改写这些语义事实或决定 mandatory fact 是否保留。用户消息的 source fact 固定为 `user_request`，但 summary item 可用 `sourceFactIds` 把它分类到 `userConstraints` 或 `decisions`；mandatory coverage 对分类项只计算 `sourceFactIds`（缺省时才使用 `factId`），validator 逐一验证原 source fact 的 canonical text 与 evidence，因此分类 ID 不能借用其他 mandatory fact。Observation 的 `keyFacts` 必须与 ledger canonical text 精确一致。候选结果依次经过 JSON/Zod、完整 provenance 集合精确匹配、fact 语义与 evidence binding、mandatory fact IDs、user message coverage 和基于同一次 effect、同一 projection 输入计算的 before/after token gain 术语（压缩前后文本计量收益），并只允许一次 schema repair 术语（模式修复）。Manual 与 auto 的统一最低绝对缩减为 1024 token 术语（文本计量单位），target ratio 术语（目标比例）只作诊断。M2 的可靠 observation 复用 `ResourceObservationTracker`：截断、legacy digest 或缺少 raw digest/revision 的 projected digest 不构成可靠观察，path write、未知范围 write 和 workspace mutation 会使旧观察失效。只有超出单次摘要输入预算时才按完整 turn/tool block 分块；chunk summary 术语（分块摘要）只是 merge 术语（合并）输入，只有最终 summary 能成为 checkpoint 术语（检查点）。

Model Controller 术语（模型控制器）在 Provider 调用前通过统一的 `buildContextProjection()` 入口计算五级 context pressure 术语（上下文压力）：`normal / warning / compact_due / hard_limit / unknown`。这些级别只用于诊断与自动触发启发式；`hard_limit` 不阻止普通模型调用。正常模型调用、compaction effect 术语（压缩副作用）与 `/context` 通过同一个 `resolveContextProjectionEnvironment()` 重建当前 built-in tools、已绑定 MCP tools、tool search、workflow skill descriptors 与 active inline skill instructions，并共享包含 adapter metadata 的 resolved model capabilities；before/after 必须共享该环境，正式 acceptance 术语（验收）不读取旧 preflight 的 estimate。自动模式为 `off | shadow | live`，原因只允许 `manual | auto`；`triggerRatio` 或 `triggerTokens` 命中后仍受 cooldown 术语（冷却）、收益预检和 thrash breaker 术语（频繁压缩熔断器）约束。自动失败只记录失败并可更新 breaker，不创建 hard block 术语（硬阻断）。`ContextHardBlock` 只保留 Runtime correctness failure 术语（运行时正确性故障）原因，普通压缩成功或 reset 不清除它。已有 checkpoint 时执行增量压缩；Core 不从通用 Provider HTTP 400 或错误文本推断 overflow 术语（上下文溢出）。Active checkpoint 只替换模型投影中的 covered prefix，原始 transcript 继续由 RuntimeStore 保存。Compaction lifecycle、context pressure、token savings 与真实 effect duration 写入 session trace，不能只依赖进程内指标。
