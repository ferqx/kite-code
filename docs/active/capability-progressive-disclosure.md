...
27/19/8 parity由Builtin/SPI package tests与当前Runtime manifest checks机械验证；RM-16 final Gate、完成记录
...
snapshot；package tests机械断言27 entries、19 model-visible、8 internal及identity/effects/schema parity；
`git_inspect`已从 Builtin registry 退役；Git 操作通过受治理的 `shell_execute` 执行。
