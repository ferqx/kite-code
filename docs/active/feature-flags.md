...
验证：`bun test apps/kite-cli/test/config/features.test.ts`、`bun run test:tui:system prompt-contract-v2-production`。

Runtime 功能开关注册在 `apps/kite-cli/src/config/features.ts`。配置从用户级和项目级 `kite-code.jsonc` 的可选 `features` 对象读取；项目值覆盖用户值。
