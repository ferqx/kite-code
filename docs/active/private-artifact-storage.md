...
验证：`bun test tests/private-immutable-artifacts.test.ts tests/model-artifacts.test.ts tests/runtime/capability-artifacts.test.ts tests/persistence/filesystem-preimage-artifacts.test.ts tests/subagent-artifacts.test.ts`、`bun run typecheck`、`bun run check:core-boundary`。
