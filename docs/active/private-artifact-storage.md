...
验证：`bun test packages/builtin-runtime/test/isolated/private-immutable-artifacts.test.ts tests/integration/model-artifacts.test.ts tests/isolated/runtime/capability-artifacts.test.ts packages/builtin-runtime/test/persistence/filesystem-preimage-artifacts.test.ts apps/kite-cli/test/subagent-artifacts.test.ts`、`bun run typecheck`、`bun run check:core-boundary`。
