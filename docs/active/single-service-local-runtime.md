...
`run/resume`切到parent-owned stdio App Server；single-Service只在显式legacy `service *`命令中可达，且不是fallback。KASD-01已完成
...
single-Service implementation，不属于普通启动路径。KASD-04增加显式本机App Server daemon，KASD-05已把Web listener/client迁入该daemon；
当前没有旧库导入、dual write或fallback。
...
`kite-app-server-daemon-v2`与capability集合判断兼容，build ID只作诊断；相同protocol的不同build可连接，v1或未知required capability拒绝。
v2 status携带strict loopback `webOrigin`；同进程listener提供exact assets、API Docs与Browser read-only `/v1`。普通client/Browser disconnect
不停止daemon，显式stop才关闭Web admission、取消active Turn、drain carrier并清理endpoint。自定义POSIX endpoint只接受已存在的
canonical owner-only parent，status不创建owner state，不兼容或identity不确定的owner不会被替换或停止。

`service *`仍显式进入legacy single-Service控制面，等待KASD-06删除；`kite web`只读现存daemon且不隐式start。下文关于legacy Native endpoint、
build replacement与Store 9的描述只约束该显式legacy路径，不能作为默认TUI/CLI fallback。

legacy Service拥有一个Runtime Host、一个Store 9 writer connection和一个仅供Native Runtime/health/control的loopback listener。Workspace仍是
Trust、配置、Skill、MCP、Sandbox、Controller和query scope，但不拥有独立进程、DB或idle lifecycle。Service根、assets与Browser `/v1`
保持404，不读取Web asset root。
...
## 显式daemon Web与legacy Service启动

- source `bun run server`先build Web，再显式执行daemon start与`kite web`；direct `kite server start`在assets缺失时fail closed。installed daemon
  只从launcher-pinned candidate的`payload/web`解析assets。daemon endpoint ready时Web listener、static/API revision与status origin已经共同ready。
- `GET /`、`GET /api-docs`与受限Session shell返回同一个SPA index并创建或复用read-only HttpOnly Browser session；同origin `/v1`直接复用
  daemon的`kite-session.sqlite` Runtime/History/Directory/Checkpoint owner，不打开第二DB。Browser logout只撤销cookie，不停止daemon。
- `kite web [--server <endpoint>]`只调用daemon status并打印`webOrigin/`；absent提示显式start，v1/v2 mismatch或identity uncertain保持
  fail closed，不spawn、replace或stop。TUI `/status`从不显示Web URL。
- legacy Native IPC只保留`describe/service_stop`；其`httpOrigin`仅是旧Runtime WebSocket/control transport地址，不是Web root。
- legacy `service ensure`的同home并发请求经过同一manager/reservation，只产生一个ready Service；custom `--kite-home`只在同canonical
  legacy profile内复用。只读Native `describe`即使`expectedBuildId`不同也返回兼容Service的真实identity；manager保留
  该actual build作部署决策，不能把describe成功等同于build已收敛。显式shared source只复用`dev:`→`dev:` drift；
...
Browser打开URL不拥有本机启动权限；Vite dev server只提供前端资源。source `bun run server`build Web assets并显式启动daemon；
