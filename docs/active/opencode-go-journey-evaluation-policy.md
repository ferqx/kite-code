...
`safe_pre_dispatch_transient` 在真实 MCP readiness 边界、任何 capability dispatch 之前记录首次 `provider_unavailable`，其 canonical authority 固定为 `not_started/none/pre_dispatch`；只有 RuntimeStore durable `tool.retry_recorded` ack 后才执行第二次 readiness attempt 与唯一一次 capability dispatch，并同时报告失败 authority/timing/resolution 与成功 `recoveryOf`。该 fixture 必须使用 canonical descriptor revision 与 Runtime 签发的 turn binding；任意 binding 或 schema digest 不能冒充 admission 证据，报告只能从持久化的 readiness intent、waiter、attempt、terminal receipt 与 retry lineage 推导。`sandbox_permission_no_escalation` 使用 `tests/helpers/sandbox-executor.ts` 的显式 fail-only oracle 与 `shell_execute` 边界，固定产生 `sandbox_error/sandbox_denied`；同一 helper 注入可触发、调用即写 synthetic marker 的受控 bare-shell sentinel，case 必须观测一次 sandbox boundary、零 sentinel/底层命令调用、零 authorization-widening Runtime event 和零 replay。该 case 只是不降级回归 oracle，不构成 production `SandboxExecutionProviderV1` 或平台资格证据。不可观测的“权限提升尝试数”不得硬编码进报告，也不得用 approval rejection 或 phase rejection 冒充。`repeated_failure_replan_finalize` 在同 lineage 完成两次真实 read/search failure 后，通过 production `write_plan` structural replan、plan review、`update_plan` finalize 收敛，两个历史 failure 只能由显式 `replanned` resolution 消除。`timeout_unknown_no_replay` 保留 V2 CompletionGuard 两次 stable block 与同批 `turn.aborted + run.error` 证据。
