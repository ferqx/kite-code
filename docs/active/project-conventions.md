...
Required workflow 的其他独立 job 不改变上述分支控制语义。当前版本没有 evaluation/record/replay job；
production restore/replay 语义与测试门禁仍由各自 Runtime 测试覆盖，不能冒充 evaluation evidence。
...
- **用内联 `import()` 规避循环依赖**：内联 import 不应作为逃避架构问题的手段。若低层 package 需要引用高层 package 或 App 类型，说明类型定义放错了层级——应移到 `runtime-contract` 或 `runtime-spi` 的中立边界。
...
客户端可见模式常量定义在 `packages/runtime-contract/src/presentation.ts`，Runtime State 内部枚举由
