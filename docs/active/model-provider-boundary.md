...
模型响应流式能力优先从显式模型配置、adapter runtime metadata 或 `modelKwargs.streaming` 解析；缺失时默认 `true` 且不伪造 source，用户无需配置。正常 Agent 调用使用单步 `streamText`，以累计全文语义实时发出不可持久化的 `model.text_delta` / `model.reasoning_delta`；每段连续 reasoning 另发一次带稳定 `segmentId` 的 `model.reasoning_completed`，Provider 缺少显式 start/end 时由 adapter 在 reasoning→text/tool/流结束边界合成。三种瞬态事件不进入 Kernel reducer、event store、snapshot 或 Session Logger，但会以 closed local event 进入前台 TUI reducer；流结束后仍只由 durable `model.responded` 推进 canonical transcript、工具分发和轮次状态。History Client 从该 durable completion 合成等价 completed reasoning/text 序列，不持久化每个 delta。显式 `streaming: false` 时使用 `generateText`；summary/reviewer 等内部模型调用不切换到此 TUI 流式路径。详见 ADR-0034、ADR-0045、ADR-0143。
