...
Summary model 的受支持边界仍是 provider-neutral AI SDK、确定性 temperature、无工具、SDK 零重试和 bounded output。progressive manual/auto、V3 writer、资源终态、Working Set、branch transaction、resumable migration 与 stale/fork recovery 已通过本地 PSMC-06 Gate；auto rollout flag 仍默认关闭，本地资格不代表 production/default-on。该路线不得保存 Provider 原始响应、授权、Plan、Verification 或 Tool authority，Session Memory 不参与。Compaction effect 不读取旧 `lastPreflight` 参与 acceptance。
...
压缩原因解析 `manual | auto`，但新 auto orchestrator 尚未资格化。本地 estimate 不能证明 Provider admission、阻止普通模型请求或创建 `ContextHardBlock`。`ContextHardBlock` 只接受 Runtime correctness failure 术语（运行时正确性故障）原因。
