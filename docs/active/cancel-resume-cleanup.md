...
释放 successor 的外部执行门禁或回退 host Shell。TUI 只展示 `sandbox | denied` startup
决策；不得用 UI 卸载、重建 executor 或新 session 清除 durable cleanup authority。
