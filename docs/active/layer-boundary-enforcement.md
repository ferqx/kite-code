...
当前 Runtime State production caller/owner closure 已切换到 App RuntimeSessionCoordinator、Host generic coordinator、Kernel
与 Builtin；RM-16 最终 manifest/docs/journey/fault/soak Gate 和完成证据已经闭合。RA 只能沿现有依赖方向增加
...
Runtime State Kernel facts、Runtime Event 与 provider interface 只由对应 package seam 导出，不把 Host/Builtin/App authority
倒灌进 Contract。
...
PS-01/RM-12 还增加 filesystem seam 的静态所有权：只有规范 package 路径
...
RM-13 增加 Shell/Sandbox 的静态所有权：`SandboxExecutionProvider` 只定义在
...
改为已删除并让 verifier 证明不可达。`architecture-exceptions.json` 只允许精确、带 owner 和到期 RM Task
的 compatibility edge，禁止目录级 allowlist。

RM manifest 最初只冻结 package/owner/source facts；RA 后同一生成机制的当前输出机械显示 Runtime State、SQLite Store 与
`kite-runtime-modularization-v1-2026-08-19`，并从 production SQLite Store source 绑定未被旧 header shim 占用的
`.runtime-state-store.db` target path。清单不能把历史 Runtime State/SQLite Store checkpoint 或 `.runtime-state-store.db`
header shim 冒充当前 production truth。

### RM-03 workspace 与 Client authority 边界

仓库现有六个私有 Runtime package 与 `apps/kite` 都具备真实源码、显式 exports、consumer test、独立
build/typecheck/test。RM-03 已把 CLI/TUI、Git、Observability、Release 与 Workspace App 源码迁入
...
`createKiteRuntimeBoundary`、`runCli` 和 `runTui`；过渡 `src/index.ts` 必须不存在，公共入口不能构造或重新暴露
...
和已删除的 ProjectHandle/DataOrigin/EgressAuthority/内部 key 协议泄漏仍须由该 Gate 拒绝；Runtime State/SQLite Store
...
SessionManager 与 CLI 当前只经 `@kite/runtime-host` 的窄 `projectRuntimeObservabilityFact` port 调用同一
...
SQLite Store production adapter，且只能导入 Host storage exports。`apps/kite/src/bootstrap.ts` 是唯一 concrete
...
Runtime State storage view 只由 Host storage port 提供；root tests 通过 Host Runtime State codec 与 SQLite SQLite Store adapter 组合同一个 seam，
不再复制 concrete driver。session-specific format preflight 必须发生在写连接建立之前。四类 transaction port
都只允许映射到一次既有原子提交，禁止 fallback、dual write、retry-on-alternate-driver 或扩展 SQLite Store。

### RM-05 Host、Session 与通知边界
...
列表和一个惰性 SQLite Store owner；Kernel session 取得的是同一 storage 的引用计数 view，view 关闭只释放 lease，Host dispose 后在
...
effect/cancellation/recovery ownership 仍属于 RM-06；RM-05 不新增 AbortController、lease、
late receipt 或 unknown recovery 实现。

### RM-06 Host lifecycle 与 recovery 边界

`@kite/runtime-host` 唯一拥有长期 operation root AbortController、same-session cleanup barrier、shutdown drain、
四类 SQLite Store transaction acknowledgement、单-Store effect lease supervision 与 restart recovery。cancel fact 在
...
### RM-07 Pure Kernel 与 AuthorizedEffect 边界

`@kite/agent-kernel` 是 production transition owner，只导出纯 `decide/reduce/selectPendingEffects`、结构化
State/Event/Effect contract、`KernelInput`、canonical `DecisionFacts` 与 RM 最小 `AuthorizedEffect`。package
...
Host 唯一拥有 Contract Command 到 `KernelInput` 的翻译。当前 Runtime State domain reducer/scheduler 通过 package 内固定
reducer 组合接入纯 transition；该 binding 不得持久化、执行 effect 或形成第二 decision path。旧 Core Kernel control
surface 已删除，Runtime State restore/recovery 由 RuntimeSessionCoordinator 与 Host session seam 负责。

`apps/kite/src/bootstrap/runtime/KiteRuntimeExecutionModule.ts` 及其 registered handler 是当前唯一 App execution
bridge。它只消费 Host/Kernel 已形成的 Runtime State facts 与 single handler route，不得重新 classify、policy、approve、
...
### RM-08 Runtime SPI、Registry 与 App Execution Module 边界

`@kite/runtime-spi` 是私有编译边界，不是公开 Plugin ABI 或恶意同进程隔离。它冻结 `RuntimeModule`
...
App、Node/Bun ambient authority 或具体 builtin。RM DTO 不含 RA 的 ProjectIdentity、DataOrigin、Credential、
签名、Runtime State 或 SQLite Store 字段。
...
dispose 先关闭 bridge，再反向释放 module，最后关闭 SQLite Store storage。Host 仍只拥有通用机制，不解释具体
...
bridge。production caller/owner closure 与 RM-16 最终 Required Gate 已闭合。
...
已经全部通过；RM-16 与其 manifest 已有完成证据。

### RM-09 Capability lifecycle 与 name-free Scheduler 边界

`@kite/runtime-spi` 的 `CapabilityBinding` 必须与当前 Runtime State turn binding 逐字段一致；Disclosure、Proposal、
Intent、AuthorizedEffect 与 Receipt 是分离 DTO，不能把 model proposal 当 grant。`RuntimeModuleRegistry.snapshot()`
返回 immutable definition/executor/context-source identity，`arbitrateCapability()` 只能返回 exact resolved identity
或 typed failure；它不得读 Session approval、调用 Provider 或签发 Grant。

`@kite/builtin-runtime#createCapabilityBinding` 是唯一 production binding 构造者，保留既有 canonical digest
字节；root catalog 的同名 helper 只是 RM-16 前的兼容入口。RM-10 已迁移的 `tool_search` executor 见下一节；
其余 concrete capability executor 仍由后续 vertical slice 按 owner manifest 迁移。

`@kite/agent-kernel` 拥有纯 `ExecutionTraits` overlap/batch decision。Builtin catalog declaration 只声明静态 resource scope、
conflict/isolation/barrier/group/lease facts，兼容投影器再从当前 call 的已持久化 effect classification 与 causal
identity 补齐 traits；这些 traits 不加入 Runtime State 或 RuntimeEvent。`packages/agent-kernel/src/scheduler.ts` 只能消费 traits
...
### RM-10 Host execution port 与 `tool_search` vertical slice 边界

`@kite/runtime-spi` 只新增 provider-neutral 的 `CapabilityExecutionInvocation`/`CapabilityExecutionPort`；
冻结的非 authority catalog facts 属于 `ExecutionRequest.facts`，不在 execution context 增设旁路字段。Provider
context 精确只有 grant、request digest、signal、selected execution environment 与 attempt identity；受限进程内
机制只能挂在允许的 `ExecutionEnvironmentRef`，不能作为另一个 Runtime Provider 或持久 authority。
...
不接收 Workspace、MCP/Skill runtime 或 Model handle。RM-10 checkpoint 后遗留的 `tool_search` compatibility
...
生产链固定为 Registry/Proposal/Policy → Intent → SQLite Store invocation+attempt 原子 ack → Host port → 唯一 Builtin
...
identity 缺失都 fail closed，不得回到已删除的 central executor。当前 Runtime State、SQLite Store、epoch
`kite-runtime-modularization-v1-2026-08-19` 保持既有 approval/readiness/安全语义，并叠加 Runtime State/SQLite Store 格式验证与 canonical Workspace identity；不存在 Runtime installation root 或全局 Host lock。

### RM-11 Skills、Context ports、MCP 与 Web ownership 边界
...
RM-11 没有引入通用 DataOrigin/Egress IR；这些推测性 contract 后续也已按 ADR-0127 删除。
Credential IR 只服务真实 OAuth/API secret。project approval、endpoint revision、transport recovery、Runtime State、
SQLite Store 与 epoch `kite-runtime-modularization-v1-2026-08-19` 保持各自 owner。

### RM-12 Filesystem 与 Git ownership 边界
...
没有 `execute/projectResult`；旧 Provider/Git/protocol 路径已删除。Host port 在 Tool Pipeline 已完成 resolve/validate/classify/authorize/admit、SQLite Store
...
unknown recovery。Runtime State、SQLite Store 与 epoch `kite-runtime-modularization-v1-2026-08-19` 未变化；
canonical Workspace identity、strict SQLite Store 和真实 CredentialBroker 是当前边界，cross-Host fence 与
DataOrigin/Egress IR 不再属于 production design。

### RM-13 Shell 与 Sandbox ownership 边界
...
### RM-14 Verification 与 Subagent ownership 边界
...
JSON/cursor、role/ceiling 与 lifecycle 语义以及 `BuiltinChildRuntimeDriver`。App 的 verification/subagent adapter 只把
Runtime State 投影为最小 view、注入 Shell/MCP/Artifact/reviewer port，并把 Builtin check result 转成既有
`verification.*` event；repair、compensation、Verification Policy 与 Completion 仍由 Kernel/Runtime 原权威处理。
App composition root 只构造一个 `BuiltinChildRuntimeDriver` 与 governed composition。当前
...
canonical Workspace identity、strict control frame、真实 CredentialBroker 与 Runtime State/SQLite Store；ProjectHandle、
single-Host global lock、内部 key/HMAC 与 DataOrigin/Egress IR 已删除。

### RM-15 Model、Context、Compaction 与 Reviewer ownership 边界
...
Host 只持有 `ContextCompilerPort` 与 effect lifecycle，不解释 Model、Prompt、Context 或 Reviewer 语义。
...
与 `turn-coordinator.ts` 是唯一 production Runtime State effect/caller seam，并复用同一 Gateway、Builtin coordinator、catalog、
Host capability port、投影环境与 SQLite Store effect lease。compaction terminal batch 只以 exact lease identity 持久化一次；
不存在 Core controller/executor/subagent caller、第二 coordinator 或 fallback。RM-16 源码 closure 与最终
...
compaction acceptance 与 reviewer failure propagation 进入唯一 production composition。Runtime State、SQLite Store 和 epoch
...
- `runtime-spi` 定义 neutral `RuntimeControlFrame`；Host 在真实 child process channel 上验证 exact
...
- `createSqliteRuntimeStorage`、Runtime State codec 与 target path 由 App bootstrap 唯一组合；SQLite adapter 是
  Runtime State/SQLite Store 唯一 persistence owner，public package 只导出当前 constructor/path/metadata constants。
- `preflight.ts` 只读验证当前格式，`adapter.ts` 唯一拥有连接生命周期；不存在旧 constructor、旧 path、fallback 或格式选择分支。
- SQLite Store exact DDL 与 manifest 固定为 7 tables / 2 indexes；SQLite target commit 是 snapshot metadata 的最终
...
> Session domain cutover: `apps/kite/src/runtime/session/` owns Runtime Session and SessionManager; `apps/kite/src/adapters/tui/runtime-bridge.ts` is the TUI adapter. Runtime Session imports no TUI types.
> Tool guard split: project-instruction and production execution-surface guards live under `apps/kite/src/runtime/tool-execution/` and do not own Policy or persistence.
