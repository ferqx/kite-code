...
`/compact` 触发上下文压缩并支持可选的自定义摘要指令（例如 `/compact focus on auth changes`）。当没有足够的历史消息时返回"Not enough messages to compact."。命令通过 live Kernel control 写入 compaction request event；interaction/tool/verification barrier 优先，compaction 排队而不会创建并发 Kernel。
