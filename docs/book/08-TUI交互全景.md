...
`/compact` 触发上下文压缩并支持可选的自定义摘要指令（例如 `/compact focus on auth changes`）。当没有足够的历史消息时直接返回"Not enough messages to compact."并通过 `compaction_failed` 事件持久化到 transcript。会话切换后 slash command handler 通过 ref 保持最新，避免 Ink 7 `useInput` 闭包过期。
