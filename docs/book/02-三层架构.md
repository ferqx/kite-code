...
packages/agent-kernel/                  无外部 I/O 的 State 25 Kernel、Event、Reducer、Scheduler
...
packages/runtime-storage-sqlite/        Store 4 SQLite storage adapter
...
- `apps/kite` 是唯一 composition root，组合同一个 frozen registry snapshot、Builtin projection、MCP facts、Host port 与 Store 4 adapter。
...
| Runtime Kernel | `agent-kernel` | 保存 State 25 事实，调度 Effect，通过 Event 推进状态 |
...
RuntimeState
  ...
  → 产生 State 25 RuntimeEvent
  → agent-kernel Reducer 更新 RuntimeState
  → runtime-storage-sqlite Store 4 持久化
```
