...
5. 当前 production-supported platform 集合为空：
   - 本机 macOS 探针证明 network-off，但现有 profile 仍允许 Workspace 外、`.git` 和
     symlink escape 读写，且没有硬 process-tree 上限、完整继承或入口组合证据；allowlist 也
     尚不可用，但当前矩阵选择 network-off，因此它是能力限制而不是本次排除的单独原因；
   - Linux 与 Windows hosted runner 的原生 evidence artifact 尚未固定；
   - 没有 verified in-process read-only fallback。
6. `.github/workflows/platform-capability-probe.yml` 必须在声明支持的 runner 上生成原生 evidence
   ...
   已固定、矩阵字段全部满足且 ADR 转为 accepted 后，独立 release gate 才能关闭 D-04 的平台
   部分并把对应组合加入 production support set。`backend=none` 不得输出进程型 `supported`。
7. 本 ADR 仍为 proposed、原生 artifact 尚未固定且 D-04 开放，因此 Task 1B.1–1B.4 保持
   blocked；当前只允许继续完善 bounded spike、原生 evidence 与决策材料，不能开始 backend
   production 实现、产生 production 资格或把任何平台标为支持。
...
当前发布支持矩阵保持空集合，D-04 继续开放，1B 后续工作不会提前解锁 production artifact。
探针会保守地暴露缺口；新 backend、runner 或 composition root 必须重新产生完整证据。
