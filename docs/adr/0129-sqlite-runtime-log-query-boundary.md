...
2. `@kite/runtime-contract` 拥有 transport-neutral 的 cursor、完整性、展示和 typed-error DTO；`@kite/runtime-host/storage` 单独拥有 generic `RuntimeLogQueryPort`。它不属于 `SessionStore`，不提供 transaction、effect、checkpoint、delete 或 Artifact reader。
3. `@kite/runtime-storage-sqlite` 以 `SQLITE_OPEN_READONLY | SQLITE_OPEN_NOFOLLOW` 打开独立短生命周期 reader，先进行 current-format preflight，随后以 current codec 解码 event。旧 epoch、未知或损坏 event 均 fail closed；不会返回 `event_json`、建表、写库或引入 compatibility decoder。
