...
| PS-03 | in_progress | Protocol-first grant/handle/observation、Pipeline-owned runtime、唯一 Local Provider、ChildRuntimeDriver、normal/resume/Skill 三路 cutover、single-use/expiry/Fake negatives、bounded cancel 与 no-bypass 已接线；真实 Pipeline planning/protected-path、approval resume 与 child MCP retry parity 通过。独立 private immutable task Artifact publication/readback、跨进程 durable Provider handle restore/reconcile 与完整 record→replay start/resume qualification仍未闭合，不能标 completed；schema v24/epoch 未改变 |
