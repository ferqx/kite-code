...
| RP-00 | next | 新增 Replay ADR 与 active evaluation policy，收敛 cassette content domain、approved suite authority 和 risk-based promotion criteria；本任务不接入 replay source |
| RP-01、Provider seams、CUT-01 | pending | `ModelAttemptOutcomeV1` source/catalog、Local Filesystem/Sandbox/Subagent Provider 与唯一 production epoch cutover 尚未实施；当前 Model evidence 只有 live source，不得声称 replay 或新 production epoch 已完成 |
