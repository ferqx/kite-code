...
最后更新：2026-09-02（接受App Server进程与Durable Session解耦方案，显式daemon/Web取代默认全局single-Service ownership）
...
|------|------|--------|------|-----------|----------|
| [`2026-09-02-app-server-session-decoupling.md`](2026-09-02-app-server-session-decoupling.md) | active | P0 | ADR-0166、Store 9、Runtime Server/Client、Controller/effect receipt current authority | 接管Kite Home单Service计划的未完成与后续演进；保留durable Store成果，替代global Service/Runtime/Store/Web共生命周期 | Session writer fencing、同build local App Server、TUI/CLI cutover、显式daemon、Web client解耦、旧build convergence删除 |
