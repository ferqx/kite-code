...
最后更新：2026-08-30（Kite Agent Server API V1 完成02D read-only conformance/fault Gate）
...
|------|------|--------|------|-----------|----------|
| [`2026-08-29-kite-agent-server-api-v1.md`](2026-08-29-kite-agent-server-api-v1.md) | active | P1 | ADR-0149/0150、KRSV1/KCWW 与当前 Runtime/Store/Controller/Web authority | 在 private Runtime Protocol 之上新增 stable local REST/SSE façade；不开放 remote、Browser mutation、public `/rpc` 或第二 Runtime/Store | KASAPI-00A～02D已完成；Public-codec reference client已关闭auth/pagination/concurrent update/limits/drain/role/replacement/non-disclosure Gate。当前进入KASAPI-03A/KRSRUN-01A Store 8 schema；Run mutation与`runs`capability继续blocked至子计划完成 |
| [`2026-08-29-kite-runtime-run-store-v1.md`](2026-08-29-kite-runtime-run-store-v1.md) | active | P1 | ADR-0150、KASAPI-00C/02D、Store 7 current authority | KASAPI-03A子计划：State 27 / Store 8 canonical Run index、receipt resource result、coverage boundary与offline generation migration | KRSRUN-00A文档freeze与父计划02D Gate已完成；当前执行01A neutral port/Store 8 DDL/index/preflight，不开放Store 7 partial Run或历史event推断回填 |
