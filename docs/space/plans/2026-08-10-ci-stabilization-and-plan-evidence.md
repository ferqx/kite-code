...
**目标：** 先以不弱化 CompletionGuard V1 的方式恢复 PR #46 的 Required CI，再实现 `ACORE-PLAN-02` 的 schema/evidence，最后以 `ACORE-PLAN-03` 单独升级 CompletionGuard V2。
...
- [x] **Step 1：逐条复现 CI 的五个 unit/E2E 与五个 PTY failure group**
...
- [x] **Step 2：先把 fixture 的 completion candidate 改为可观察的 V1 blocker**
...
- [x] **Step 3：运行定向测试确认 fixture 在新语义下失败转绿**
...
- [x] **Step 2：实现 V2 schema、Artifact write/read 区分与严格 transition**
...
- [x] **Step 3：实现 evidence 的 Runtime provenance 投影**
...
- [x] **Step 4：验证 V2 与 legacy replay 边界**
...
- [x] **Step 5：更新当前 Plan 文档并提交 schema tranche**
...
### Task 3：ACORE-PLAN-03 / 将 CompletionGuard 升级为 V2 verification/evidence gate
