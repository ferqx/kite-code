...
将审批交互模式从仅 CLI/配置文件可设，扩展为 TUI 内 `/permissions` 快捷指令可随时切换，并重新设计审批确认面板。
...
- **`/permissions` 快捷指令** — 支持 `ask/auto/full` + 简写 `a/f`，无参循环切换
...
- **移除 `/auth`** — 功能合并到 `/permissions full`
...
- **full = autonomous + full_access**：`/permissions full` 同时设置交互模式和授权模式
- **审批面板联动**：按 `Full` 自动切换 `/permissions full`；按 `Auto` 自动切换 `/permissions auto`
...
| `tests/tui-slash-command.test.ts` | +8 | /permissions 解析（含简写 a/f/au） |
