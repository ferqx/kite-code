...
Fail-closed：target adapter 的 format marker 必须为 schema 5/target epoch；旧 session 不通过 target path restore，缺失或不匹配 marker 直接抛出 typed format incompatibility。没有旧/new fallback。
