...
本地 Gate：frozen install、format/lint、7 workspace typecheck/build/test、runtime package/core/manifests、全仓 default、完整 TUI system（39 isolated PTY files，含 keyless upgrade）、fault、local CI soak、installed OSS candidate（含 invocation-local MCP wrapper）需在本 ADR-0126 实现收敛后重新记录最终计数与 SHA。
