...
启动首先接入原生宿主持有的 App Server；没有服务时可以不指定执行工作区启动。目录、持久会话投影、订阅初始快照与历史均从同一 profile Store 读取，不解析旧项目的文件系统，不要求 Git、模型配置或 Workspace Trust 可用。模型、项目和分支读取在目录之外完成，各自报错；失效路径仍可阅读历史和继续原会话；发送不要求Desktop登记目录或目录仍存在，由Service按持久Session身份核对既有授权。
...
点击任意空间的已有会话只切换读取与订阅，不关闭原 Service，不停止正在执行的任务。已有会话直接按原Session ID提交给Service，不切换执行项目、不执行本地目录预检；目录缺失和项目未登记不禁用发送。Service按持久空间身份与既有授权执行，目录相关能力在工具调用时失败，不改用其他cwd。草稿按原会话ID隔离，项目登记变化不改变草稿键。新对话的显式项目选择仍核对目录与授权。已有会话的权限设置使用原连接，由 Service 按持久会话身份核对目标工作区信任并路由，不调用执行项目切换或停止任务。普通资料目录可执行，无须 Git；分支功能仅适用于 Git 仓库。
...
操作反馈由 [OperationToast](../src/OperationToast.tsx) 使用 Sonner 在会话主区域顶部居中展示，取代此前“操作未完成”模态弹窗；旧 Figma 恢复弹窗不再代表当前交互。错误和恢复警告具有类型文字、图标和颜色，不遮罩页面、不接管焦点。通知保留到关闭或问题清除，“检查恢复”只在恢复警告上显示，不自动发送草稿。

2026-09-17 修正已完成历史会话的遗留执行权阻断：发送时由 Service 事务核验完整收尾事实，满足条件直接在原 sessionId 追加新一轮。历史工具在派发前被拒绝且不会重试的记录不再单独触发恢复弹窗；真实未知结果或尚未收尾仍保留恢复提示。

会话重新进入的旧执行校准由 Service 统一处理，Desktop 不依据断线或关闭次数取消任务。服务端核验并追加的子 Agent 终态经既有历史／实时事件投影显示；历史子 Agent 缺 parentToolCallId 时，Service 使用持久 Provider childInvocationId 的唯一关联核验，不按显示名称推断。运行中的另一 Service 会话保持真实进度，资源核对失败仍可阅读历史。
