...
只读目录请求有 10 秒边界，可重试一次；明确不可重试错误不会重试。选择会话的查询、订阅 ready 与历史加载／投影共用 20 秒边界；完整校准前禁止发送和交互操作，连接快照不能提前恢复 ready。选中代次隔离迟到结果；读取失败保留已读正文，明确不存在、空间身份不匹配或 unauthorized 则清除正文。
