...
开发配套服务准备会在发布候选签名及摘要校验之后，对暂存文件再次校验 macOS 签名，再以 rename 替换可执行文件，避免原地覆盖已执行的 inode。旧候选若签名无效则在准备阶段报错，不在客户端启动时重签或修改候选身份。2026-09-14 本机 Bun 1.4.0 编译产物曾被 macOS 以 `SIGKILL (Code Signature Invalid)` 终止；修复后候选测试与真实 Electron 配套服务连接、重接、历史和 EOF 清理验收通过。这仍是本机开发证据。
