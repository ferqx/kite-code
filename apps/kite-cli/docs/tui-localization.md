...
- `/status`的命令说明、App Server transport/profile/build/version、client version与`same_build|exact_protocol` pairing提示使用
...
- 旧`serviceStatus.*`、PID/build-drift启动告警与`tui:fresh`文案全部删除；`/status` unavailable只使用
  `appServerStatus.unavailable`，不能恢复Service身份分支。
