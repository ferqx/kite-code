...
import { sessionLogDir, sessionLogFrontendDir, sessionLogRoot } from '#kite-cli/config/paths';
import type { SessionLoggingPolicy } from '#kite-cli/config/session-logging-policy';
import { SessionLogCollector } from '#kite-cli/session-logger/collector';
import { SessionLogWriter } from '#kite-cli/session-logger/writer';
...
describe('secure session log writer', () => {
  ...
  test('rejects a linked session root before creating frontend/session directories', () => {
    if (process.platform === 'win32') return;
    const root = isolateHome();
    const kite = join(root, '.kite-code');
    const outside = join(root, 'outside-sessions');
    mkdirSync(kite, { mode: 0o700 });
    mkdirSync(outside, { mode: 0o700 });
    symlinkSync(outside, join(kite, 'sessions'));
  ...
  test('rejects a linked user data directory before creating the session root', () => {
    ...
    mkdirSync(outside, { mode: 0o700 });
    symlinkSync(outside, join(root, '.kite-code'));
  ...
  test('protects a live lease held by a separate process', async () => {
    ...
    const child = Bun.spawn({
      cmd: [process.execPath, 'tests/fixtures/session-logger/lease-holder.ts'],
      cwd: process.cwd(),
      env: { ...process.env },
      stdin: 'pipe',
      stdout: 'pipe',
