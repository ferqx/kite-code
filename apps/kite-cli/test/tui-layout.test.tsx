...
} from '../src/tui/components/BlockRenderer';
import HelpPanel from '../src/tui/components/HelpPanel';
import InputBlock from '../src/tui/components/InputBlock';
...
import DiffPreview from '../src/tui/DiffPreview';
import Footer from '../src/tui/Footer';
import Header, { formatHeaderWorkspace } from '../src/tui/Header';
      ...
      tools: [
        {
          callId: 'c1',
          name: 'read_file',
          args: { path: 'apps/kite-cli/src/tui/App.tsx' },
          ok: true,
          summary: 'ok',
          status: 'done',
        },
        {
          callId: 'c2',
          name: 'read_file',
          args: { path: 'apps/kite-cli/src/tui/types.ts' },
          ok: false,
          summary: '',
          status: 'running',
        },
...
describe('Block spacing', () => {
  ...
  test('joins a streamed item after a mixed block whose final component is a list', () => {
    const blocks: OutputBlock[] = [
      {
        id: 1,
        kind: 'text',
        content: '应用层在 apps/kite-cli/src/：\n\n- tui/ — React Ink TUI 主界面\n',
      },
    ...
    const lines = (lastFrame() ?? '').split('\n');
    const tui = lines.findIndex((line) => line.includes('tui/ — React Ink'));
    const cli = lines.findIndex((line) => line.includes('cli/ — Headless'));
    const runtime = lines.findIndex((line) => line.includes('runtime/ — App Runtime'));
...
describe('App', () => {
  ...
  test('renders children when provided', () => {
    const state = fakeState();
    const InputLine = require('../src/tui/components/InputLine').default;
    const { lastFrame } = render(
      <App state={state} dispatch={noop} onToggleReason={noop} provider={fakeProvider()}>
        <InputLine mode="prompt" onSubmit={noop} workspace={process.cwd()} />
      </App>,
