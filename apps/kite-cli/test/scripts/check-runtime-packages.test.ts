...
describe('runtime workspace package gate', () => {
  ...
  test('rejects unexported deep imports', () => {
    const root = createFixture();
    updateText(root, 'apps/kite-cli/src/bootstrap.ts', (value) =>
  ...
  test('rejects a second concrete App composition root', () => {
    const root = createFixture();
    const path = join(root, 'apps/kite-cli/src/alternate.ts');
  ...
  test.each([
    ...
    const root = createFixture();
    const path = join(root, 'apps/kite-cli/src/alternate-authority.ts');
