...
import { appApprovalBindingForPresentation } from '#kite-cli/bootstrap/runtime/approval-binding';
import {
...
} from '#kite-cli/bootstrap/runtime/subagent/tool-adapter';
...
const unitToolDispatchers = new WeakMap<
  object,
  import('#kite-cli/bootstrap/runtime/subagent/types').SubAgentToolDispatcher
>();
const TEST_RECOVERY_IDENTITY_KEY = '7'.repeat(64);
type TestSubAgentRunnerInput = Omit<
  import('#kite-cli/bootstrap/runtime/subagent/types').SubAgentRunnerInput,
...
function directUnitToolDispatcher(input: {
  ...
  const dispatcher: import('#kite-cli/bootstrap/runtime/subagent/types').SubAgentToolDispatcher = {
...
function mockEventSink() {
  ...
  return {
    ...
    sink: ((e: { type: string; data: Record<string, unknown> }) => {
      events.push(e);
    }) as unknown as import('#kite-cli/bootstrap/runtime/subagent/types').SubAgentEventSink,
  };
}
...
describe('SubAgentRunner integration', () => {
  ...
  test('classifies the role deadline separately from parent cancellation', async () => {
    ...
    const descendantResourceAdmission = {
      ...
      import('#kite-cli/bootstrap/runtime/subagent/types').SubAgentRunnerInput['descendantResourceAdmission']
    >;
