...
import { bindAppApprovalBinding } from '#kite-cli/bootstrap/runtime/approval-binding';
import {
  classifyFailure,
  classifyMcpProviderError,
  failureKindForToolParseFailure,
} from '#kite-cli/bootstrap/runtime/failures';
import { createAppMcpReadinessRuntime } from '#kite-cli/bootstrap/runtime/mcp-readiness-runtime';
import { createPlanRuntime } from '#kite-cli/bootstrap/runtime/plan-runtime';
import {
  type ProviderReadinessCoordinator,
  ProviderReadinessPersistenceError,
  ProviderReadinessUnknownError,
} from '#kite-cli/bootstrap/runtime/provider-readiness';
import type { RuntimeEvent, RuntimeState } from '#kite-cli/bootstrap/runtime/state-runtime';
import type { SubAgentResult } from '#kite-cli/bootstrap/runtime/subagent/types';
import type { AppToolPipelineComposition } from '#kite-cli/bootstrap/runtime/tool-pipeline-composition';
import {
  type AppOrdinaryToolPipelineAttemptRuntime,
  isAppOrdinaryToolPipelineOperationId,
} from '#kite-cli/bootstrap/runtime/tool-pipeline-ordinary-attempt';
import type { AppTaskToolPipelineAttemptRuntime } from '#kite-cli/bootstrap/runtime/tool-pipeline-task-attempt';
import {
  buildToolApproval,
  commandIdentityForToolApproval,
} from '#kite-cli/bootstrap/runtime/tool-policy';
import {
  createSkillMechanismPort,
  createWebMechanismPort,
} from '#kite-cli/bootstrap/runtime/tool-provider-services';
import type { ToolExecutionResult } from '#kite-cli/bootstrap/runtime/tool-result';
import { createAppToolTurnContext } from '#kite-cli/bootstrap/runtime/tool-turn-context';
import { getFeatureFlags } from '#kite-cli/config/features';
import { type AgentConfig, computeExecutionBoundaryDigest } from '#kite-cli/config/index';
import {
  policyRecoveryTerminal,
  productionExecutionSurfaceFailure,
  sealedMcpNetworkTerminal,
} from '#kite-cli/runtime/tool-execution/execution-surface-guard';
import { projectInstructionGuardFailure } from '#kite-cli/runtime/tool-execution/project-instruction-guard';
import { appPreparedShellExecutionPort } from '#kite-cli/sandbox/prepared-tool-pipeline';
import type {
...
export async function executeAppRuntimeTools(params: {
  ...
  subagentRuntimeFactory?: import('#kite-cli/bootstrap/runtime/subagent/pipeline-runtime').AppSubagentRuntimeFactory;
