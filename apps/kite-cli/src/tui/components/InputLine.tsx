...
import { useFileSearch } from '#kite-cli/tui/hooks/useFileSearch';
import {
  SLASH_COMMAND_DEFS,
  SLASH_COMMANDS,
  type SlashSuggestionData,
  useSlashSuggestions,
} from '#kite-cli/tui/hooks/useSlashSuggestions';
...
export type { SlashSuggestionData } from '#kite-cli/tui/hooks/useSlashSuggestions';
  ...
  const handleSubmit = useCallback(
    (val: string) => {
      ...
      if (ps) {
        ...
        if (idx >= 0) {
          finalValue =
            val.slice(0, idx) + ps.pastedContent + val.slice(idx + ps.placeholder.length);
        } else {
          finalValue = val;
        }
      } else {
      ...
      if (finalValue.length > MAX_INPUT_LENGTH) {
        ...
        setInputWarning(
          `Input too large (${(finalValue.length / 1000).toFixed(0)}KB > ${(MAX_INPUT_LENGTH / 1000).toFixed(0)}KB limit)`,
        );
        return;
      }
  ...
  useInput(
    (
      ...
      if (ss.active && ss.result && dismissedSlashValueRef.current !== valueRef.current) {
        ...
        if (key.downArrow) {
          ss.setSelectedIndex((s: number) => Math.min(ss.result!.items.length - 1, s + 1));
          return;
        }
