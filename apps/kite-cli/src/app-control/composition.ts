...
import { defaultCheckpointPath, skillDirs } from '#kite-cli/config/paths';
import { composeObservability } from '#kite-cli/observability/composition';
...
>(
  ...
  const ownersFor = (workspace: KiteWorkspaceIdentity) => {
    ...
    if (current) {
      ...
      const refreshed = probeAgentConfig({ workspace: workspace.canonicalPath });
    ...
    const configProbe = probeAgentConfig({ workspace: workspace.canonicalPath });
    ...
    const created = Object.freeze({
      ...
      skills: createSkillCatalogOwner({ workspace }),
