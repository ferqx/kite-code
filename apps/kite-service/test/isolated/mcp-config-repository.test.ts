...
describe('MCP config repository', () => {
  ...
  test('keeps explicit Service user and approval paths independent from ambient home', async () => {
    const explicitUserPath = join(root, 'service-state', 'mcp.json');
    const explicitApprovalPath = join(root, 'service-state', 'approvals.jsonc');
    mkdirSync(join(root, 'service-state'), { recursive: true });
    writeFileSync(
      explicitUserPath,
      JSON.stringify({ mcpServers: { explicit: { command: 'explicit-command' } } }),
    );
    writeFileSync(
      join(home, '.kite-code', 'mcp.json'),
      JSON.stringify({ mcpServers: { ambient: { command: 'ambient-command' } } }),
    );
    writeFileSync(explicitApprovalPath, '{ malformed');
    writeFileSync(
      join(workspace, '.kite-code', 'mcp.json'),
    ...
    const repository = new DefaultMcpConfigRepository({
      ...
      approvalPath: explicitApprovalPath,
    });
    const catalog = await repository.load(workspace);
    expect(catalog.effective.has('explicit')).toBe(true);
    expect(catalog.effective.has('ambient')).toBe(false);
    expect(catalog.projectApprovals[0]?.status).toBe('store_corrupt');

    await repository.mutate({
      type: 'add',
      scope: 'user',
      name: 'written-explicitly',
      config: { type: 'stdio', command: 'service-command' },
      expectedRevision: catalog.sourceRevisions.user,
    });
    expect(readFileSync(explicitUserPath, 'utf8')).toContain('written-explicitly');
    expect(readFileSync(join(home, '.kite-code', 'mcp.json'), 'utf8')).not.toContain(
      'written-explicitly',
    );
  });
});
