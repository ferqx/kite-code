...
import { SessionLogCollector } from '#kite-service/session-logger';
import { openStateStoreForTest } from '../../../../../scripts/support/runtime-storage';
import { CURRENT_TEST_PLAN_REVIEW_FACTS } from '../../../../../tests/helpers/current-plan';
import { createMockModel } from '../../../../../tests/helpers/mock-model';
import { runTestRuntimeAgent } from '../../../../../tests/helpers/runtime-model';
import { buildRunAgentParams } from '../../../src/bootstrap/runtime/runtime-agent-input';
...
describe('session logger composition', () => {
  ...
  test('writer construction failure reports once and does not stop the Runtime', async () => {
    ...
    const blockedCodeRoot = join(blockedHome, 'kite-code-root');
    writeFileSync(blockedCodeRoot, 'not a directory');
    const previousHome = process.env.KITE_CODE_HOME;
    process.env.KITE_CODE_HOME = blockedCodeRoot;
    const diagnostics: string[] = [];
    const events: RuntimeEvent[] = [];
    try {
      for await (const event of runTestRuntimeAgent(
