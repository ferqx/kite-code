...
} from '../../src/runtime/session/contracts';
import {
...
import { appSandboxBackendAvailable } from '../../src/sandbox/types';
...
/**
 * Service tests need the provider port but must not import the CLI/TUI provider.
 * This fixture keeps the same identity and generation checks at the injected
 * boundary while leaving all presentation code in the CLI-owned tests.
 */
class TestSessionUserInputProvider implements SessionUserInputProvider {
  private pendingResolve: ((action: SessionUserAction) => void) | null = null;
  private pendingInterrupt: SessionInterruptPayload | null = null;
  ...
  submitAction(action: SessionUserAction | Record<string, unknown>): void {
  ...
  reset(): void {
    ...
    this.pendingResolve = null;
    this.pendingInterrupt = null;
    if (resolve && interactionId) resolve({ type: 'cancel', interactionId });
  }

  private normalizeAction(
    action: SessionUserAction | Record<string, unknown>,
  ): SessionUserAction | null {
    const pending = this.pendingInterrupt;
    const type = action.type;
    const interactionId =
      typeof action.interactionId === 'string' ? action.interactionId : pending?.interactionId;
    if (!interactionId || typeof type !== 'string') return null;
    if (
      (type === 'approve' || type === 'reject') &&
      pending?.kind === 'approval' &&
      (interactionId !== pending.interactionId || action.generation !== pending.generation)
    ) {
      return null;
    }
    switch (type) {
      case 'approve':
        if (typeof action.generation !== 'number' || typeof action.grant !== 'string') return null;
        return {
          type,
          interactionId,
          generation: action.generation,
          grant: action.grant as ApprovalAction['grant'],
        };
      case 'reject':
        if (typeof action.generation !== 'number') return null;
        return { type, interactionId, generation: action.generation };
      case 'input':
        ...
        return {
          ...
          interactionId,
          text: action.text,
          ...(action.answers && typeof action.answers === 'object'
            ? { answers: action.answers as Record<string, string> }
            : {}),
        };
      case 'cancel':
        return { type, interactionId };
      case 'plan_review_decision':
        if (!action.decision || typeof action.decision !== 'object') return null;
        return {
          type,
          interactionId,
          decision: action.decision as PlanReviewAction['decision'],
        };
      default:
        return null;
    }
...
type Action = SessionPresentationAction;
type ApprovalAction = Extract<SessionUserAction, { type: 'approve' }>;
type PlanReviewAction = Extract<SessionUserAction, { type: 'plan_review_decision' }>;
...
function makeDeps(checkpointPath = ':memory:'): SessionDeps {
  ...
  return {
    config,
    provider: new TestSessionUserInputProvider(),
    skillManifests: [],
    skillOptions: null,
    mcpManager: null,
    checkpointPath,
...
function makeStatus(overrides: Partial<SessionStatusProjection> = {}): SessionStatusProjection {
  return {
    phase: 'building',
    plan: null,
    pendingPlan: null,
...
describe('durable TUI approval action bridge', () => {
  test('sends exact approval actions once and emits identity-bound input/plan actions', async () => {
    const provider = new TestSessionUserInputProvider();
