...
} from '#kite-service/runtime/tool-persistence';
...
describe('App State Tool Pipeline persistence', () => {
  test('binds durable filesystem intent to the exact prepared acknowledgement and current State record', async () => {
  ...
  test('issues and verifies only the latest authentic State read observation query result', async () => {
    const harness = persistenceHarness();
    const candidate = prepared(1);
    const acknowledgement = await harness.persistence.recordAttempt(candidate);
  ...
  test('commits the authentic read observation into the capability receipt, artifact, and State', async () => {
    const harness = persistenceHarness();
    const candidate = prepared(1);
    const acknowledgement = await harness.persistence.recordAttempt(candidate);
    const operation = readOperation();
  ...
  test('commits task start approval and auto-review suspensions as one State batch', async () => {
  ...
  test('keeps a sibling task approval in the durable queue when the focus is occupied', async () => {
    ...
    seeded.processEvent({
      ...
      toolCallId: 'occupied-call',
      fullModeBypassEligible: occupiedApproval.fullModeBypassEligible,
      fullModePolicyBypassAllowed: occupiedApproval.fullModePolicyBypassAllowed,
      approvalRoute: 'user',
      queueGeneration: occupiedApproval.queueGeneration,
      queueSequence: occupiedApproval.queueSequence,
    ...
    const acknowledgement = await harness.persistence.recordAttempt(taskPrepared(1));
    const suspension = taskSuspension('start', 'approval', 1);
    ...
    expect(harness.calls.receiptEvents[0]?.[2]).toMatchObject({
      type: 'approval.requested',
    ...
    expect(harness.session.getState().suspendedSubagents['call-1']).toEqual(suspension.subagent);
    expect(harness.session.getState().pendingApprovals.get('interaction-task-1')?.status).toBe(
      'awaiting_user',
    );
    expect(harness.session.getState().tools.calls['call-1']?.status).toBe('awaiting_approval');
  });
  ...
  test('commits task resume approval and auto-review suspensions only after exact prior suspension state', async () => {
    for (const eventKind of ['approval', 'auto_review'] as const) {
      ...
      const resumeSuspension = taskSuspension('resume', eventKind, 1);
      await harness.persistence.commitSuspension({
  ...
  test('rejects stale, skipped, and cross-parent task resume suspension state before Store4', async () => {
    ...
    const validResume = taskSuspension('resume', 'approval', 1);
    const malformed = [
      {
        ...validResume,
  ...
  test('rejects forged or non-safe retry evidence before another State attempt is authorized', async () => {
