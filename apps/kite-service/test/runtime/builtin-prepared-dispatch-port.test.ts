...
} from '#kite-service/bootstrap/runtime/builtin-prepared-dispatch-port';
import type {
  AppPreparedToolInvocationPacket,
  AppToolPipelinePreparedRequest,
} from '#kite-service/bootstrap/runtime/tool-pipeline-prepared';
import type {
...
describe('App Builtin prepared dispatch port', () => {
  ...
  test('does not call the supplied Host port directly or create a second registry', async () => {
    const source = await Bun.file(
      new URL('../../src/bootstrap/runtime/builtin-prepared-dispatch-port.ts', import.meta.url),
    ).text();
