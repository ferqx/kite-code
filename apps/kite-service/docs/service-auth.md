...
| App Server daemon Runtime/control | 无secret token | owner-only Unix socket/current-user named pipe；首请求initialize；fixed exact daemon v2/capabilities |
| App Server daemon Browser | HttpOnly SameSite cookie | loopback exact Host/peer与same-origin Fetch Metadata；只读`/v1`，不能进入Native/Runtime control |
