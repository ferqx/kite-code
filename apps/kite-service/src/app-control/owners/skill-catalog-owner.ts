...
import { type SkillDirsOverrides, skillDirs } from '#kite-service/config/paths';
...
export interface SkillCatalogOwnerOptions {
  ...
  /** Explicit Service user skill roots; project roots always derive from the admitted path. */
  readonly skillOptions?: SkillDirsOverrides;
}
...
export function createSkillCatalogOwner(input: SkillCatalogOwnerOptions): SkillCatalogOwner {
  ...
  const refresh = (): readonly SkillManifest[] => {
    const scanned = scanCompiledSkillManifests(
      skillDirs(input.workspace.canonicalPath, input.skillOptions),
    );
