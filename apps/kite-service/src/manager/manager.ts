...
function diagnosticForHandshake(
  ...
  if (runtime.expectedBuildId !== undefined && handshake.buildId === undefined) {
    return 'identity_uncertain';
  }
  return undefined;
}
...
function buildMismatch(
  ...
  if (runtime.expectedBuildId === undefined) return false;
  return handshake.buildId !== runtime.expectedBuildId || descriptor.buildId !== handshake.buildId;
}
...
export function createKiteServiceManager(options: KiteServiceManagerOptions): KiteServiceManager {
  ...
  const probeExisting = async (
    ...
    const diagnostic = diagnosticForHandshake(handshake, runtime);
    if (diagnostic) {
      if (handshake.outcome !== 'unavailable') {
    ...
    return result(
      ...
      'ready',
      descriptor,
      buildMismatch(descriptor, handshake, runtime) ? 'build_mismatch' : undefined,
    );
  };
