...
export function blockedSubagentReviewEvent(input: {
  ...
  parentToolCallId: string;
  blocked: NonNullable<
    import('#kite-service/bootstrap/runtime/subagent/types').SubAgentResult['blocked']
  >;
  availCtx: AppToolTurnContext;
...
export async function executeAppTaskToolPipeline(input: {
  ...
  result = await taskRuntime.execute({
    ...
    threadId: state.session.threadId,
    ...(params.commandContext === undefined ? {} : { commandContext: params.commandContext }),
    attempt: expectedAttempt,
    ...
    capabilityExecution,
    ...(params.workspaceEffect === undefined ? {} : { workspaceEffect: params.workspaceEffect }),
    signal: params.signal ?? new AbortController().signal,
    workspace: state.session.workspace,
    phase,
    executionMode,
