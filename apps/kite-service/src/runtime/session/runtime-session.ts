...
} from '#kite-service/bootstrap/runtime/command-interaction-decision';
import type {
...
import { buildRunAgentParams } from '#kite-service/bootstrap/runtime/runtime-agent-input';
...
import type { PrecommittedStartTurnDescriptor } from '#kite-service/bootstrap/runtime/turn-command-decision';
...
import { composeAppGitBroker, resolveAppGitExecutable } from '#kite-service/git/composition';
import type { RuntimeMetricBridge } from '#kite-service/observability/runtime-bridge';
import {
...
export interface SessionDeps {
  ...
  /** The only Store 4 production constructor, injected by the Service bootstrap. */
...
export class SessionRuntime {
  ...
  private _routeRuntimeEvent(
    ...
    if (this._foreground) {
      ...
      // projector. The Service runtime projector owns the raw→safe projection and emits an
