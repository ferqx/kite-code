...
} from '#kite-service/bootstrap/runtime/file-checkpoints';
import { loadSession } from '#kite-service/bootstrap/runtime/session-persistence';
import type {
  RuntimeEvent,
  RuntimeState,
  StateRuntimeStorage,
...
export interface RewindServiceDependencies {
  ...
  /** Borrowed Service-owner storage; callers must not close it. */
...
export class RewindService {
  ...
  constructor(deps: RewindServiceDependencies) {
    if (!deps.storage && !deps.openStateRuntimeStorage) {
      throw new TypeError('Rewind Service requires one storage access port.');
    }
    this.deps = deps;
  }

  private open(threadId?: string): { store: StateRuntimeStorage; owned: boolean } {
    return this.deps.storage
      ? { store: this.deps.storage, owned: false }
      : { store: this.deps.openStateRuntimeStorage!(threadId), owned: true };
  }

  listCheckpoints(threadId: string) {
    const { store, owned } = this.open(threadId);
    try {
  ...
  async executeIntent(input: {
    ...
    const { intent } = input;
    const { store, owned } = this.open(intent.sourceSessionId);
    try {
      ...
      if (restoresConversation) {
        ...
        recoveredData = await loadSession(
          this.deps.storage
            ? () => nonClosingStorage(this.deps.storage!)
...
function nonClosingStorage(store: StateRuntimeStorage): StateRuntimeStorage {
  return Object.freeze({
    sessions: store.sessions,
    transactions: store.transactions,
    effects: store.effects,
    checkpoints: store.checkpoints,
    recoveryIdentities: store.recoveryIdentities,
    close: () => undefined,
  });
}
