...
export function loadMcpConfigCatalog(
  ...
  const store = readProjectMcpApprovalStore(options.approvalPath);
  if (!workspaceKey) {
    diagnostics.push({
  ...
  return {
    ...
    sourceRevisions: Object.freeze({
      ...
      user: sourceTextRevision(userPath),
    }),
  };
}
