...
import type { ToolApprovalPayload, UserInputPayload } from '@kite-ai/runtime-contract';
import type { DurableSuspendedSubagent } from '@kite-ai/runtime-spi';
import {
  CURRENT_TEST_PLAN_IDENTITY,
  CURRENT_TEST_PLAN_REVIEW_FACTS,
} from '../../../tests/helpers/current-plan';
import { currentRuntimeEvent } from '../../../tests/helpers/current-runtime-event';
import type { Action } from '../src/tui/App';
import { eventReducer as canonicalEventReducer, createInitialState } from '../src/tui/App';
import {
  projectToolCancelled,
  projectUserCancelledTurn,
} from '../src/tui/reducers/cancellation-projection';
import { buildToolSummaryLine } from '../src/tui/reducers/consolidateTools';
import {
  handleRuntimeEventAction as handleCanonicalRuntimeEventAction,
  handleEventAction,
  type RenderEvent,
} from '../src/tui/reducers/handleEvent';
import type {
  InterruptState,
  OutputBlock,
  SessionSnapshot,
...
describe('eventReducer (blocks model)', () => {
      ...
      s = dispatch(s, {
        ...
        event: {
          ...
          data: {
            ...
            outputTokens: 30,
            hitRate: 0.5,
            standard: {} as import('@kite-ai/runtime-contract').PromptCacheStandardEvaluation,
          },
        },
      });
  ...
  describe('RUNTIME_EVENT message-list pipeline', () => {
    ...
    test('keeps approval in Footer while user input and plan review use their own projections', () => {
      const approvalEvent: import('@kite-ai/agent-kernel').RuntimeEvent = {
        type: 'approval.requested',
        ...APPROVAL_EVENT_METADATA,
        interactionId: 'approval-1',
        toolCallId: 'write-1',
      ...
      const inputEvent: import('@kite-ai/agent-kernel').RuntimeEvent = {
        type: 'user_input.requested',
        interactionId: 'input-1',
        toolCallId: 'ask-1',
        request: question(),
      };
      const reviewEvent: import('@kite-ai/agent-kernel').RuntimeEvent = {
        type: 'plan.review_requested',
    ...
    test('updates cache statistics and keeps non-visual lifecycle facts out of the message list', () => {
      let state = fresh();
      const nonVisual: import('@kite-ai/agent-kernel').RuntimeEvent[] = [
        { type: 'turn.started', turnId: 'turn-1' },
        { type: 'model.requested', requestId: 'request-1' },
