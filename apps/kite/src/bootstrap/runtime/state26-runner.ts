import {
  ...
  runtimeHostState26DecideCompletionV1 as decideCompletion,
  type State26RuntimeEffectExecutorV1 as HostState26RuntimeEffectExecutorV1,
  isRuntimeHostState26ToolRecoveryInvalidV1,
  isState26RuntimeEffectDeferredV1,
  planRuntimeBudgetAdmissionV1,
  type State26RuntimeEffectV1 as RuntimeEffect,
  reconciliationEventsForReservationsV1,
  type State26RuntimeSchedulerFactsV1 as SchedulerFactsV1,
  type State26RuntimeEffectLeaseV1,
...
import type { RuntimeActionResult, RuntimeUserAction } from './state26-actions';
...
type RuntimeEffectExecutorV1 = HostState26RuntimeEffectExecutorV1<
  RuntimeState,
  RuntimeEvent,
  RuntimeEffect
>;
...
/**
 * Transitional State26 runner port. Production is backed by the one Host
 * session; Core's historical AgentKernel only remains a test migration source.
 */
export interface RuntimeState26SessionPortV1 {
  ...
  beginEffect(effect: RuntimeEffect): State26RuntimeEffectLeaseV1;
  isEffectEventCurrent(lease: State26RuntimeEffectLeaseV1, event: RuntimeEvent): boolean;
  applyEffectEvent(lease: State26RuntimeEffectLeaseV1, event: RuntimeEvent): boolean;
  applyEffectResult(
    lease: State26RuntimeEffectLeaseV1,
    events: RuntimeEvent[],
  ...
  releaseEffect?(lease: Readonly<State26RuntimeEffectLeaseV1>): void;
}
...
async function* executeEffectWithStreaming(
  kernel: RuntimeState26SessionPortV1,
  ...
  if (isRuntimeHostState26ToolRecoveryInvalidV1(kernel.getState())) {
 ...
 */
export async function* runState26RuntimeLoopV1(
  kernel: RuntimeState26SessionPortV1,
  executor: RuntimeEffectExecutorV1,
