...
export const APP_STATE25_TOOL_PIPELINE_PERSISTENCE_SCHEMA_V1 =
  'kite.app-state25-tool-pipeline-persistence.v1' as const;

/** State25 consumes the Builtin-owned neutral result; App defines no second schema. */
export type State25BuiltinOperationStructuredContentV1 = BuiltinOperationExecutionValueV1;
type AuthenticatedFilesystemObservationV1 = Extract<
  BuiltinWorkspaceFilesystemTerminalVerificationResultV1,
  { readonly valid: true }
>['observation'];

export interface AppState25ToolPipelinePersistenceV1
  extends ToolPipelinePersistenceV1<State25BuiltinOperationStructuredContentV1> {
  ...
  /** State25 scan only; Builtin validates the returned Artifact and decides stale_read. */
...
export interface CreateAppState25ToolPipelinePersistenceInputV1 {
  readonly getState: () => Readonly<State25RuntimeStateV1>;
  readonly persistAttemptStartEvents: (events: State25RuntimeEventV1[]) => Promise<boolean>;
  readonly persistTerminalRecoveryEvents: (events: State25RuntimeEventV1[]) => Promise<boolean>;
  readonly persistReceiptEvents: (events: State25RuntimeEventV1[]) => Promise<boolean>;
  readonly now: () => string;
...
export type AppState25ToolPipelinePersistenceErrorCodeV1 =
...
export class AppState25ToolPipelinePersistenceErrorV1 extends Error {
  readonly code: AppState25ToolPipelinePersistenceErrorCodeV1;

  constructor(code: AppState25ToolPipelinePersistenceErrorCodeV1, message?: string) {
    super(message ?? appState25ToolPipelinePersistenceMessageV1(code));
    this.name = 'AppState25ToolPipelinePersistenceErrorV1';
    this.code = code;
  }
}

export function createAppState25ToolPipelinePersistenceV1(
  input: Readonly<CreateAppState25ToolPipelinePersistenceInputV1>,
): AppState25ToolPipelinePersistenceV1 {
  ...
  const recordAttempt = async (
    ...
    if (identity.attemptId !== expectedAttemptId) {
      throw new AppState25ToolPipelinePersistenceErrorV1(
        'attempt_identity_mismatch',
        'Prepared attemptId does not match the State25 next attempt suffix.',
      );
    }

    const now = state25TimestampV1(input.now());
    ...
    const events: State25RuntimeEventV1[] = [];
  ...
  const persistFilesystemIntent: WorkspaceFilesystemDurableEvidencePortV1['persistIntent'] = async (
    ...
    if (
      ...
      throw new AppState25ToolPipelinePersistenceErrorV1('filesystem_intent_invalid');
    }
    ...
    if (
      ...
      throw new AppState25ToolPipelinePersistenceErrorV1('filesystem_intent_commit_failed');
    }
  ...
  const persistFilesystemMutationReady: WorkspaceFilesystemMutationDurableEvidencePortV1['persistMutationReady'] =
    async (draft) => {
      ...
      if (
        ...
        throw new AppState25ToolPipelinePersistenceErrorV1('filesystem_mutation_ready_invalid');
      }
      assertOpenAcknowledgementV1(
        issuedAcknowledgements,
        settledAcknowledgements,
  ...
  const recordUnknown = async (unknown: Readonly<ToolPipelineUnknownOutcomeV1>): Promise<void> => {
    ...
    const finishedAt = state25TimestampV1(input.now());
    ...
    const events: State25RuntimeEventV1[] = [
      ...
      },
      {
        type: 'tool.failed',
        toolCallId: identity.toolCallId,
        failure: runtimeHostState25ClassifyFailureV1(
    ...
    if (
      ...
      throw new AppState25ToolPipelinePersistenceErrorV1(
        'acknowledgement_mismatch',
        'State25 unknown terminal acknowledgement did not commit the exact invocation.',
      );
    }
  ...
  const commitRetryable = async (
    commit: Readonly<ToolPipelineRetryableCommitV1<State25BuiltinOperationStructuredContentV1>>,
  ): Promise<void> => {
    ...
    if (!prepared) {
      throw new AppState25ToolPipelinePersistenceErrorV1('acknowledgement_mismatch');
    }
    const request = readPreparedRequestV1(prepared);
    ...
    const classifiedFailure = runtimeHostState25ClassifyFailureV1(
      'provider_unavailable',
      failure.message,
    );
    const outcome = runtimeHostState25ClassifyToolOutcomeV1({
      status: 'failed',
    ...
    if (
      ...
      throw new AppState25ToolPipelinePersistenceErrorV1('retryable_commit_failed');
    }
    ...
    const recoveryOf = runtimeHostState25ToolFailureInstanceIdV1({
      toolCallId: identity.toolCallId,
      invocationFingerprint,
      outcome,
    });
    const candidateJournal = runtimeHostState25RecordRecoveryFailureV1(before.toolRecovery, {
      toolCallId: identity.toolCallId,
      toolName: call.name,
      invocationFingerprint,
    ...
    const admission = runtimeHostState25AdmitRecoveryAttemptV1(candidateJournal, {
      toolCallId: identity.toolCallId,
      toolName: call.name,
      invocationFingerprint,
      modelMessageId: call.modelMessageId,
    ...
    if (!admission.admitted || admission.recoveryOf !== recoveryOf) {
      throw new AppState25ToolPipelinePersistenceErrorV1(
        'retryable_commit_failed',
        'State25 recovery policy did not admit the exact safe-read retry.',
      );
    }
    const retryEvent: State25RuntimeEventV1 = {
    ...
    if (
      ...
      throw new AppState25ToolPipelinePersistenceErrorV1(
        'retryable_commit_failed',
        'State25 did not acknowledge the exact safe-read retry evidence.',
      );
    }
  ...
  const commitTerminal = async (
    commit: Readonly<ToolPipelineReceiptCommitV1<State25BuiltinOperationStructuredContentV1>>,
  ): Promise<void> => {
    const identity = assertSupportedAcknowledgementV1(commit);
    assertOpenAcknowledgementV1(
      issuedAcknowledgements,
    ...
    const topLevelTaskRecoveryEvents = runtimeEvents.filter(
      (
        event,
      ): event is Extract<State25RuntimeEventV1, { type: 'subagent.recovery_journal_merged' }> =>
  ...
  const commitSuspension = async (
    commit: Readonly<ToolPipelineSuspensionCommitV1<State25BuiltinOperationStructuredContentV1>>,
  ): Promise<void> => {
    const identity = assertSupportedAcknowledgementV1(commit);
    assertOpenAcknowledgementV1(
      issuedAcknowledgements,
    ...
    if (
      ...
      throw new AppState25ToolPipelinePersistenceErrorV1('invalid_suspension_result');
    }
    const { runtimeEvents, value } = readStructuredContentV1(commit.result);
    if (
    ...
    const recordedAt = state25TimestampV1(input.now());
    ...
    const siblingCancellationDecisions = runtimeHostState25PlanReviewSiblingCancellationsV1(
      before,
      identity.toolCallId,
    );
    const siblingCancellations = siblingCancellationDecisions.map(
      (decision): State25RuntimeEventV1 => ({
        type: 'tool.cancelled',
...
function isExactDynamicMcpRetryableFailureValueV1(
  value: State25BuiltinOperationStructuredContentV1 | undefined,
...
interface CommitTaskSubagentSuspensionInputV1 {
  readonly commit: Readonly<
    ToolPipelineSuspensionCommitV1<State25BuiltinOperationStructuredContentV1>
  >;
  readonly identity: Readonly<ToolPipelineAttemptAcknowledgementV1['attempt']>;
  readonly before: Readonly<State25RuntimeStateV1>;
  readonly prepared: Readonly<PreparedToolInvocationV1> | undefined;
  readonly input: Readonly<CreateAppState25ToolPipelinePersistenceInputV1>;
}

/**
 * Commit the task-specific non-terminal hand-off through the same State25
...
async function commitTaskSubagentSuspensionV1(
  ...
  const recordedAt = state25TimestampV1(input.input.now());
  const resultDigest = capabilityResultDigestV1(capabilityResult);
  const evidenceDigest = capabilityResultEvidenceDigestV1(capabilityResult);
  ...
  const suspendedEvent: Extract<State25RuntimeEventV1, { type: 'subagent.suspended' }> = {
  ...
  const interactionEvent: State25RuntimeEventV1 = interactionDeferred
    ? { type: 'subagent.approval_deferred', toolCallId: suspension.toolCallId }
    : (embeddedSuspensionEvent ?? (suspension.event as State25RuntimeEventV1));
...
function readTaskSuspendedContentV1(
  ...
  readonly value: Readonly<State25BuiltinOperationStructuredContentV1>;
  readonly runtimeEvents: readonly State25RuntimeEventV1[];
}> {
  if (
  ...
  let runtimeEvents: State25RuntimeEventV1[];
  try {
    runtimeEvents = admitRuntimeEventsV1(result.structuredContent.runtimeEvents);
  } catch (error) {
    if (error instanceof AppState25ToolPipelinePersistenceErrorV1) {
      throw new AppState25ToolPipelinePersistenceErrorV1(
        'invalid_suspension_result',
        'Task suspension runtime events are not valid State25 events.',
      );
    }
    throw error;
  }
...
function assertTaskSubagentSuspensionFactsV1(
  before: Readonly<State25RuntimeStateV1>,
  ...
  value: Readonly<State25BuiltinOperationStructuredContentV1>,
  runtimeEvents: readonly State25RuntimeEventV1[],
  recoveryEvent:
    | Extract<State25RuntimeEventV1, { type: 'subagent.recovery_journal_merged' }>
    | undefined,
): void {
  const call = before.tools.calls[identity.toolCallId];
  ...
  if (
    ...
    throw new AppState25ToolPipelinePersistenceErrorV1(
      'invalid_suspension_result',
      'Task suspension facts do not match the current State25 parent attempt.',
    );
  }
...
function taskSubagentRecoveryEventV1(
...
): Extract<State25RuntimeEventV1, { type: 'subagent.recovery_journal_merged' }> | undefined {
  ...
  if (!isRuntimeJsonV1(rawJournal)) {
    throw new AppState25ToolPipelinePersistenceErrorV1(
      failureCode,
      'Builtin task recovery journal is not JSON-safe State25 data.',
    );
  }
  let normalized: ReturnType<typeof runtimeHostState25NormalizeToolRecoveryJournalV1>;
  try {
    normalized = runtimeHostState25NormalizeToolRecoveryJournalV1(
      rawJournal,
  ...
  if (runtimeHostState25ToolRecoveryJournalInvalidV1(normalized)) {
    throw new AppState25ToolPipelinePersistenceErrorV1(
      failureCode,
      'Builtin task recovery journal is invalid for the current State25 identity.',
    );
  }
  const candidate = {
  ...
  try {
    const admitted = runtimeHostState25AdmitCurrentRuntimeEventV1(candidate);
    if (admitted.type !== 'subagent.recovery_journal_merged') {
      throw new Error('wrong State25 recovery event type');
    }
    return admitted;
  } catch {
    throw new AppState25ToolPipelinePersistenceErrorV1(
      failureCode,
      'Builtin task recovery event failed State25 admission.',
    );
  }
}
...
function assertTaskSubagentSuspendedStateV1(
  after: Readonly<State25RuntimeStateV1>,
  before: Readonly<State25RuntimeStateV1>,
  eventCount: number,
  identity: Readonly<ToolPipelineAttemptAcknowledgementV1['attempt']>,
  suspension: Readonly<ToolPipelineTaskSubagentSuspensionV1>,
  ...
  if (
    ...
    throw new AppState25ToolPipelinePersistenceErrorV1(
      'acknowledgement_mismatch',
      'State25 task suspension acknowledgement does not match the committed evidence.',
    );
  }
}
...
function isTaskSuspensionEventV1(
  ...
  if (!isRuntimeJsonV1(value) || !isJsonRecordV1(value)) return false;
  let admitted: State25RuntimeEventV1;
  try {
    admitted = runtimeHostState25AdmitCurrentRuntimeEventV1(value);
  } catch {
    return false;
  }
...
function assertCompositionInputV1(
  ...
  if (
    ...
    throw new AppState25ToolPipelinePersistenceErrorV1('persistence_unavailable');
  }
}
...
function assertFilesystemEditObservationQueryV1(
  ...
  if (
    ...
    throw new AppState25ToolPipelinePersistenceErrorV1('filesystem_edit_observation_invalid');
  }
}
...
function assertSupportedPreparedIdentityV1(
  ...
  if (identity.isDynamicMcp) {
    if (
      ...
      throw new AppState25ToolPipelinePersistenceErrorV1('unsupported_operation');
    }
    return identity;
  }
  ...
  if (identity.builtinProjectionRevision === null || identity.dynamicCatalogRevision !== null) {
    throw new AppState25ToolPipelinePersistenceErrorV1('attempt_identity_mismatch');
  }
  return identity;
}
...
function readPreparedRequestV1(
  ...
  if (!isPreparedRequestV1(request)) {
    throw new AppState25ToolPipelinePersistenceErrorV1(
      'invalid_prepared_request',
      'State25 persistence requires the typed App prepared request facts.',
    );
  }
  return request;
}
...
function assertPreparedStateV1(
  ...
  state: Readonly<State25RuntimeStateV1>,
  existing: Readonly<State25RuntimeStateV1['capabilities']['invocations'][string]> | undefined,
): void {
  if (
  ...
  if (
    ...
    throw new AppState25ToolPipelinePersistenceErrorV1(
      'attempt_identity_mismatch',
      'Prepared identity does not match the active State25 Tool call.',
    );
  }
  if (existing && existing.toolCallId !== identity.toolCallId) {
    throw new AppState25ToolPipelinePersistenceErrorV1('invocation_collision');
  }
  if (existing && !['recorded', 'running'].includes(existing.status)) {
    throw new AppState25ToolPipelinePersistenceErrorV1('terminal_invocation');
  }
  if (
    existing?.subagentProviderLifecycle &&
    existing.subagentProviderLifecycle.status !== 'cleanup_completed'
  ) {
    throw new AppState25ToolPipelinePersistenceErrorV1('subagent_lifecycle_pending');
  }
}
...
function invocationRecordedEventV1(
  ...
  request: Readonly<AppToolPipelinePreparedRequestV1>,
  recordedAt: string,
): Extract<State25RuntimeEventV1, { type: 'capability.invocation_recorded' }> {
  const event: Extract<State25RuntimeEventV1, { type: 'capability.invocation_recorded' }> = {
...
function assertRecordedStateV1(
  ...
  if (
    ...
    throw new AppState25ToolPipelinePersistenceErrorV1(
      'acknowledgement_mismatch',
      'State25 attempt acknowledgement does not match the committed invocation.',
    );
  }
}
...
function assertOpenAcknowledgementV1(
  ...
  if (
    ...
    throw new AppState25ToolPipelinePersistenceErrorV1(
      'acknowledgement_mismatch',
      'State25 persistence rejected an acknowledgement not issued by this owner.',
    );
  }
}
...
function assertAcknowledgementStateV1(
  state: Readonly<State25RuntimeStateV1>,
  identity: Readonly<ToolPipelineAttemptAcknowledgementV1['attempt']>,
): Readonly<State25RuntimeStateV1['capabilities']['invocations'][string]> {
  ...
  if (
    ...
    throw new AppState25ToolPipelinePersistenceErrorV1(
      'acknowledgement_mismatch',
      'State25 acknowledgement is not the current running invocation.',
    );
  }
  return invocation;
}
...
function readStructuredContentV1(
  ...
  if (!isBuiltinOperationExecutionValueV1(value)) {
    throw new AppState25ToolPipelinePersistenceErrorV1('invalid_terminal_result');
  }
...
interface LatestFilesystemObservationInvocationV1 {
  ...
  readonly artifact: NonNullable<
    State25RuntimeStateV1['capabilities']['invocations'][string]['artifact']
  >;
  readonly filesystemObservation: NonNullable<
    State25RuntimeStateV1['capabilities']['invocations'][string]['filesystemObservation']
  >;
}
...
function capabilityTerminalEventV1(
  result: Readonly<CapabilityToolTerminalResultV1<State25BuiltinOperationStructuredContentV1>>,
  value: Readonly<State25BuiltinOperationStructuredContentV1>,
  invocationId: string,
  artifact: ReturnType<CapabilityArtifactWriterV1['write']>,
  finishedAt: string,
...
function toolTerminalEventV1(
  ...
  value: Readonly<State25BuiltinOperationStructuredContentV1>,
  identity: Readonly<ToolPipelineAttemptAcknowledgementV1['attempt']>,
  resourceAdmissionFailure?: Readonly<State25ClassifiedFailureV1>,
): State25RuntimeEventV1 {
...
function fileChangeEventV1(
  ...
  value: Readonly<State25BuiltinOperationStructuredContentV1>,
  identity: Readonly<ToolPipelineAttemptAcknowledgementV1['attempt']>,
): Extract<State25RuntimeEventV1, { type: 'tool.file_change' }> | undefined {
...
function providerActionRequiredEventV1(
  composition: CreateAppState25ToolPipelinePersistenceInputV1['providerAction'],
  prepared: Readonly<PreparedToolInvocationV1> | undefined,
  result: Readonly<CapabilityToolTerminalResultV1<State25BuiltinOperationStructuredContentV1>>,
  identity: Readonly<ToolPipelineAttemptAcknowledgementV1['attempt']>,
): Extract<State25RuntimeEventV1, { type: 'provider.action_required' }> | undefined {
...
function resultMetaForToolEventV1(value: Readonly<State25BuiltinOperationStructuredContentV1>): {
...
function classifyToolFailureV1(
  ...
  const code = result.failure?.code;
  const kind = code && runtimeHostState25IsFailureKindV1(code) ? code : 'unknown';
  return runtimeHostState25ClassifyFailureV1(
    kind,
...
function assertTerminalStateV1(
  ...
  if (
    ...
    throw new AppState25ToolPipelinePersistenceErrorV1(
      'acknowledgement_mismatch',
      'State25 terminal acknowledgement does not match the committed receipt.',
    );
  }
}
...
function assertSuspendedStateV1(
  ...
  if (
    ...
    throw new AppState25ToolPipelinePersistenceErrorV1(
      'acknowledgement_mismatch',
      'State25 suspension acknowledgement does not match the committed review receipt.',
    );
  }
}
...
async function persistExactV1(
  persist: (events: State25RuntimeEventV1[]) => Promise<boolean>,
...
function state25TimestampV1(value: string): string {
...
function appState25ToolPipelinePersistenceMessageV1(
  code: AppState25ToolPipelinePersistenceErrorCodeV1,
): string {
  switch (code) {
    case 'invalid_prepared_request':
      return 'State25 Tool Pipeline prepared request facts are invalid.';
    case 'unsupported_operation':
      return 'State25 Tool Pipeline persistence does not own this operation family.';
    case 'attempt_identity_mismatch':
      return 'State25 Tool Pipeline attempt identity does not match prepared facts.';
    case 'invocation_collision':
      return 'State25 Tool Pipeline invocation identity collided with another Tool call.';
    case 'terminal_invocation':
      return 'A terminal State25 Tool invocation cannot start another attempt.';
    case 'subagent_lifecycle_pending':
      return 'A pending Subagent Provider lifecycle blocks another attempt.';
    case 'persistence_unavailable':
      return 'State25 Tool Pipeline persistence is unavailable.';
    case 'persistence_stale':
      return 'State25 Tool Pipeline persistence became stale before acknowledgement.';
    case 'acknowledgement_mismatch':
      return 'State25 Tool Pipeline acknowledgement does not match State25 state.';
    case 'filesystem_intent_invalid':
      return 'State25 filesystem intent does not match the acknowledged prepared attempt.';
    case 'filesystem_intent_commit_failed':
      return 'State25 filesystem intent could not be durably acknowledged.';
    case 'filesystem_mutation_ready_invalid':
      return 'State25 filesystem mutation ready evidence is invalid.';
    case 'filesystem_mutation_ready_commit_failed':
      return 'State25 filesystem mutation ready evidence could not be durably acknowledged.';
    case 'filesystem_edit_observation_invalid':
      return 'State25 filesystem edit observation query is invalid.';
    case 'invalid_terminal_result':
      return 'State25 Tool Pipeline terminal result is invalid.';
    case 'invalid_suspension_result':
      return 'State25 Tool Pipeline suspension result is invalid.';
    case 'artifact_write_failed':
      return 'Capability result Artifact could not be durably written.';
    case 'terminal_commit_failed':
      return 'State25 Tool Pipeline terminal receipt could not be committed.';
    case 'retryable_commit_failed':
      return 'State25 Tool Pipeline safe-read retry evidence could not be committed.';
    case 'suspension_commit_failed':
      return 'State25 Tool Pipeline suspension evidence could not be committed.';
  }
}
...
function admitRuntimeEventsV1(
  ...
  const admitted: State25RuntimeEventV1[] = [];
  for (const event of events) {
    let admittedEvent: State25RuntimeEventV1;
    try {
      admittedEvent = runtimeHostState25AdmitCurrentRuntimeEventV1(event);
    } catch {
      throw new AppState25ToolPipelinePersistenceErrorV1(
        'invalid_terminal_result',
        'Builtin runtimeEvents must be valid State25 JSON event objects.',
      );
    }
    if (
