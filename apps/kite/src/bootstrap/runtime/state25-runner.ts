import {
  ...
  runtimeHostState25DecideCompletionV1 as decideCompletion,
  type State25RuntimeEffectExecutorV1 as HostState25RuntimeEffectExecutorV1,
  isRuntimeHostState25ToolRecoveryInvalidV1,
  isState25RuntimeEffectDeferredV1,
  planRuntimeBudgetAdmissionV1,
  type State25RuntimeEffectV1 as RuntimeEffect,
  reconciliationEventsForReservationsV1,
  type State25RuntimeSchedulerFactsV1 as SchedulerFactsV1,
  type State25RuntimeEffectLeaseV1,
...
import type { RuntimeActionResult, RuntimeUserAction } from './state25-actions';
...
type RuntimeEffectExecutorV1 = HostState25RuntimeEffectExecutorV1<
  RuntimeState,
  RuntimeEvent,
  RuntimeEffect
>;
...
/**
 * Transitional State25 runner port. Production is backed by the one Host
 * session; Core's historical AgentKernel only remains a test migration source.
 */
export interface RuntimeState25SessionPortV1 {
  ...
  beginEffect(effect: RuntimeEffect): State25RuntimeEffectLeaseV1;
  isEffectEventCurrent(lease: State25RuntimeEffectLeaseV1, event: RuntimeEvent): boolean;
  applyEffectEvent(lease: State25RuntimeEffectLeaseV1, event: RuntimeEvent): boolean;
  applyEffectResult(
    lease: State25RuntimeEffectLeaseV1,
    events: RuntimeEvent[],
  ...
  releaseEffect?(lease: Readonly<State25RuntimeEffectLeaseV1>): void;
}
...
async function* executeEffectWithStreaming(
  kernel: RuntimeState25SessionPortV1,
  ...
  if (isRuntimeHostState25ToolRecoveryInvalidV1(kernel.getState())) {
 ...
 */
export async function* runState25RuntimeLoopV1(
  kernel: RuntimeState25SessionPortV1,
  executor: RuntimeEffectExecutorV1,
