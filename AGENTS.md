...
UI 层存在设计变动时，主 Agent 必须调用子 Agent，将已完成并验证的设计变动同步至对应 Figma；代码实现与 Git 操作仍由主 Agent 负责。缺少 Figma 文件或节点、写权限或可用工具时，必须说明具体阻塞，不得虚构同步结果或以其他文件替代。仅用于预览、调试且不进入产品的临时改动不触发同步。
