...
import {
  ...
  type State25RuntimeEffectPersistenceAcknowledgementV1,
...
type State25Store = RuntimeSessionStoragePortV1<KernelEvent, AgentState>;

export interface State25HarnessIdSourceV1 {
  next(scope: 'turn' | 'task' | 'kernel_runner' | 'kernel_effect' | 'model_invocation'): string;
  now(): number;
}

function liveIdSource(): State25HarnessIdSourceV1 {
...
export interface State25HostSessionHarnessInputV1 {
  readonly store: State25Store;
  readonly initialState: AgentState;
  readonly interactionMode: AgentState['mode'];
  readonly sandboxAvailable?: boolean;
  readonly runtimeIdSource?: State25HarnessIdSourceV1;
}

export class State25HostSessionHarnessV1 {
  readonly runtimeStore: State25Store;
  readonly #session: State25RuntimeSessionV1;
  readonly #runtimeIdSource: State25HarnessIdSourceV1;
  #sandboxAvailable: boolean;

  constructor(input: State25HostSessionHarnessInputV1) {
  ...
  isEffectEventCurrent(lease: State25RuntimeSessionEffectLeaseV1, event: KernelEvent): boolean {
    return this.#session.isEffectEventCurrent(lease, event);
  }

  applyEffectEvent(lease: State25RuntimeSessionEffectLeaseV1, event: KernelEvent): boolean {
  ...
  async run(
    executor: State25RuntimeEffectExecutorV1<AgentState, KernelEvent, RuntimeEffect>,
    maxEffects = 10_000,
    ...
    try {
      for (let index = 0; index < maxEffects; index++) {
        ...
        try {
          ...
          if (isState25RuntimeEffectDeferredV1(events)) {
  ...
  #id(kind: string): string {
    ...
    if (kind === 'state25_runner') return this.#runtimeIdSource.next('kernel_runner');
    if (kind === 'state25_effect') return this.#runtimeIdSource.next('kernel_effect');
    throw new Error(`Unexpected State25 harness identity scope: ${kind}`);
  }
}

export interface RestoreState25HostSessionHarnessInputV1 {
...
export function restoreState25HostSessionHarnessV1(
  input: RestoreState25HostSessionHarnessInputV1,
): State25HostSessionHarnessV1 {
  const source = input.runtimeIdSource ?? liveIdSource();
  const restored = restoreState25StateFromStoreV1({ ...input, runtimeIdSource: source });
  const harness = new State25HostSessionHarnessV1({
    store: input.store,
  ...
  const recoveryEvents = projectState25RestartRecoveryEventsV1(restored.state, {
    capabilityFinishedAtByInvocationId: Object.fromEntries(
      state25RestartRecoveryCapabilityInvocationIdsV1(restored.state).map((invocationId) => [
        invocationId,
...
export function restoreState25StateFromStoreV1(input: RestoreState25HostSessionHarnessInputV1): {
...
function compatibilityServices(
  store: State25Store,
  source: State25HarnessIdSourceV1,
