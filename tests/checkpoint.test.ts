...
describe('BunSqliteSaver', () => {
  ...
  test('persists and reads a checkpoint tuple', async () => {
    ...
    const nextConfig = await saver.put({ configurable: { thread_id: 'thread-a' } }, checkpoint, {
      source: 'input',
      step: 0,
      parents: {},
    });
...
describe('listCheckpoints', () => {
  ...
  it('returns checkpoint entries with first human message', async () => {
    ...
    const checkpoint: any = {
      ...
      channel_values: {
        messages: [
          {
            lc_id: ['langchain', 'messages', 'HumanMessage'],
    ...
    await saver.put({ configurable: { thread_id: threadId, checkpoint_id: 'cp-1' } }, checkpoint, {
      source: 'loop',
      step: 0,
      parents: {},
    });
  ...
  it('truncates long messages to 60 chars', async () => {
    ...
    await saver.put({ configurable: { thread_id: threadId, checkpoint_id: 'cp-l' } }, checkpoint, {
      source: 'loop',
      step: 0,
      parents: {},
    });
  ...
  it('returns entries in reverse chronological order', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      const cp: any = {
        ...
        ts: new Date().toISOString(),
        channel_values: {
          messages: [{ lc_id: ['langchain', 'messages', 'HumanMessage'], content: `Msg ${i}` }],
        },
        channel_versions: {},
        versions_seen: {},
      };
...
describe('getCheckpointState', () => {
  ...
  it('returns undefined authorization when missing from checkpoint', async () => {
    ...
    const checkpoint: any = {
      ...
      ts: new Date().toISOString(),
      channel_values: {
        messages: [{ lc_id: ['langchain', 'messages', 'HumanMessage'], content: 'hi' }],
      },
      channel_versions: {},
      versions_seen: {},
    };
...
describe('close safety', () => {
  ...
  it('put resolves silently after close (no throw)', async () => {
    ...
    await expect(
      saver.put(config, dummyCheckpoint, { source: 'loop', step: 0, parents: {} }),
