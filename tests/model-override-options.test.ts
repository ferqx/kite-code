...
describe('real model override options', () => {
  test('parses provider and model inline flags', () => {
    expect(parseRealTestArgs(['--provider=ollama', '--model=gemma4:31b-cloud'])).toEqual({
      providerName: 'ollama',
  ...
  test('parses provider and model space-separated flags', () => {
    expect(
      parseRealTestArgs(['--provider', 'openrouter', '--model', 'anthropic/claude-sonnet-4.5']),
    ).toEqual({
  ...
  test('rejects flags without values', () => {
    expect(() => parseRealTestArgs(['--provider'])).toThrow('--provider requires a value');
    expect(() => parseRealTestArgs(['--model='])).toThrow('--model requires a value');
  });
