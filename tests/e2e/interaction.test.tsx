...
const plan = new ResponsePlan([
  {
    ...
    responses: [
      toolCall(
        'shell_execute',
        { command: 'mkdir test-dir-1', description: 'Create test directory' },
        'Creating directory...',
        30,
      ),
  ...
  },
  {
    ...
    responses: [
      toolCall(
        'shell_execute',
        { command: 'mkdir test-dir-2', description: 'Create another test directory' },
...
describe('TUI E2E — P1 Key User Workflows', () => {
  ...
  describe('Tool Approval Flow', () => {
    test(
      'approval block appears → approve (A) → tool executes → agent continues',
      async () => {
    ...
    test(
      'approval block appears → deny (D) → tool cancelled',
      async () => {
  ...
  describe('Agent Question Flow', () => {
    ...
    test(
      'question waiting → Esc cancels → interrupt cleared',
      async () => {
  ...
  describe('Slash Suggestion Dropdown', () => {
    ...
    test(
      'typing /m shows model commands → Tab completes',
      async () => {
        clearInputBuffer(tui);
        await new Promise((r) => setTimeout(r, 100));
  ...
  describe('Slash Commands', () => {
    ...
    test(
      '/model list → model list in output',
      async () => {
    ...
    test(
      '/plan → switches to planning phase → auth set to default',
      async () => {
    ...
    test(
      '/effort → sets reasoning effort (no crash)',
      async () => {
    ...
    test(
      '/sessions → opens session selector → Esc closes',
      async () => {
  ...
  describe('@File Search', () => {
    ...
    test(
      'typing @package shows filtered results → Esc dismisses',
      async () => {
        clearInputBuffer(tui);
        await new Promise((r) => setTimeout(r, 100));
