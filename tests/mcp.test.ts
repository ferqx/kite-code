...
describe("jsonSchemaToZod", () => {
  ...
  it("maps object with properties and required", () => {
    ...
    const obj = result as z.ZodObject<z.ZodRawShape>;
  ...
  it("falls back to ZodAny for null/empty schema", () => {
    ...
    const result2 = jsonSchemaToZod(
      null as unknown as Parameters<typeof jsonSchemaToZod>[0],
    );
...
describe("expandEnvVars", () => {
  ...
  it("expands ${VAR} from process.env", () => {
    ...
    expect(expandEnvVars("prefix_${TEST_MCP_VAR}_suffix")).toBe(
  ...
  it("uses default value with ${VAR:-default} syntax when unset", () => {
    expect(expandEnvVars("${NONEXISTENT_VAR_12345:-fallback}")).toBe(
      "fallback",
    );
  });
  ...
  it("uses default value when env var is empty string", () => {
    ...
    expect(expandEnvVars("${TEST_MCP_EMPTY:-default_val}")).toBe(
      "default_val",
    );
  });
  ...
  it("handles multiple variables in one string", () => {
    ...
    expect(expandEnvVars("${TEST_MCP_VAR}_${MISSING:-beta}")).toBe(
      "alpha_beta",
    );
  });
});
