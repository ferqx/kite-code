...
describe('RP-03 approved keyless replay gate', () => {
  ...
  test('builds exact macOS and Linux isolation commands with irreversible Linux privilege drop', () => {
    const common = {
      ...
      uid: 1001,
      gid: 1002,
      linuxReadOnlyPaths: ['/usr', '/bin', '/sbin', '/lib', '/lib64', '/etc', '/sys'],
    } as const;
    ...
    expect(linux).toEqual([
      ...
      '--ro-bind',
      '/repo',
      '/kite-model-replay-workspace-v1',
      '--tmpfs',
      '/tmp',
      '--bind',
      '/tmp/private',
      '/kite-model-replay-private-v1',
      '--dev',
      '/dev',
      '--proc',
      '/proc',
      ...
      '-i',
      'PATH=/usr/bin:/bin',
      'HOME=/kite-model-replay-private-v1',
      'TMPDIR=/kite-model-replay-private-v1',
      'BUN_INSTALL_CACHE_DIR=/kite-model-replay-private-v1',
      'XDG_CACHE_HOME=/kite-model-replay-private-v1',
      '/kite-model-replay-runtime-v1/bun',
      '--no-env-file',
      '/kite-model-replay-workspace-v1/scripts/evals/run-model-replay-required-isolated.ts',
    ]);
