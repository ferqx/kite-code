...
*** End Patch`;
    ...
    const op0 = ops[0]!;
    expect(op0.kind).toBe('add');
    if (op0.kind === 'add') {
      expect(op0.file).toBe('hello.txt');
      expect(op0.lines).toEqual(['Hello, World!', 'Second line']);
    }
  });
...
*** End Patch`;
    ...
    expect(ops).toHaveLength(1);
    const op0 = ops[0]!;
    expect(op0.kind).toBe('delete');
    if (op0.kind === 'delete') {
      expect(op0.file).toBe('old.txt');
    }
  });
...
*** End Patch`;
    ...
    expect(ops).toHaveLength(1);
    const op0 = ops[0]!;
    expect(op0.kind).toBe('update');
    if (op0.kind === 'update') {
      expect(op0.file).toBe('src/app.ts');
      expect(op0.chunks).toHaveLength(1);
      const chunk = op0.chunks[0]!;
...
*** End Patch`;
    ...
    expect(op0.kind).toBe('update');
    if (op0.kind === 'update') {
      expect(op0.file).toBe('old.ts');
      expect(op0.moveTo).toBe('new.ts');
    }
  });
...
*** End Patch`;
  ...
  test('throws on missing Begin Patch', () => {
    expect(() => parsePatch('*** Add File: x.txt\n+line')).toThrow('Expected "*** Begin Patch"');
  });
