...
describe("model context protocol", () => {
  ...
  test("keeps user input as the only HumanMessage and places useful run context in SystemMessage", () => {
    ...
    expect(messages).toHaveLength(3); // 2 SystemMessages + 1 HumanMessage
    expect(messages[0]).toBeInstanceOf(SystemMessage); // 静态提示词 / Static prompt (independent cache unit)
    expect(messages[1]).toBeInstanceOf(SystemMessage); // 运行时上下文 / Runtime context (separate cache unit, session-stable)
  ...
  test("injects plan as trailing synthetic HumanMessage after conversation messages", () => {
    ...
    // 消息顺序：system, system, human(user), human(synthetic plan reminder)
    expect(messages).toHaveLength(4);
    expect(messages[0]).toBeInstanceOf(SystemMessage); // 静态提示词 / Static prompt
...
// ============================================================================
// compactOldToolResults 测试 / Tests for compactOldToolResults (append-only, cache-safe)
// ============================================================================
describe("compactOldToolResults", () => {
  test("appends runtime-state notice without modifying history", () => {
    const msgs: BaseMessage[] = [
      new HumanMessage("hi"),
      new AIMessage({
        content: "",
    ...
    const compacted = compactOldToolResults(msgs, 1);

    // 历史消息完全不变 / History messages unchanged
    expect(compacted.length).toBe(msgs.length + 1); // 追加了一条通知 / one notice appended
    expect(compacted[2]).toBe(msgs[2]); // 引用相同 / same reference
    expect(String(compacted[2] instanceof ToolMessage ? (compacted[2] as ToolMessage).content : "")).toBe("old result 1");
    expect(String(compacted[6] instanceof ToolMessage ? (compacted[6] as ToolMessage).content : "")).toBe("recent result");
    // 末尾是 runtime-state 通知 / Last message is runtime-state notice
    expect(compacted[compacted.length - 1]).toBeInstanceOf(HumanMessage);
    expect(String(compacted[compacted.length - 1].content)).toContain('<runtime-state source="harness.compaction">');
    expect(String(compacted[compacted.length - 1].content)).toContain("soft-cleared");
  });

  test("no notice when no tool results to clear", () => {
    const msgs: BaseMessage[] = [
      new ToolMessage({ content: "only result", tool_call_id: "abc", status: "error" }),
    ];

    const compacted = compactOldToolResults(msgs, 1);
    expect(compacted).toBe(msgs); // 相同引用，无操作 / same reference, no-op
  });

  test("clearOldToolResults delegates to compactOldToolResults", () => {
    const msgs: BaseMessage[] = [
      new ToolMessage({ content: "x", tool_call_id: "abc", status: "error" }),
    ];

    const cleared = clearOldToolResults(msgs, 0);
    // 追加通知但不修改原消息 / Notice appended, original message unchanged
    expect(cleared.length).toBe(2);
    expect(cleared[0]).toBe(msgs[0]);
    expect(String(cleared[1].content)).toContain("soft-cleared");
  });
});
