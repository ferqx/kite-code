...
describe('execution-boundary conformance workflow', () => {
  ...
  test('watches only current package and App owners', () => {
    ...
    expect(workflow).toContain('apps/kite-cli/src/config/execution-boundary.ts');
    expect(keyringWorkflow).toContain('packages/builtin-runtime/src/mcp/credential-store.ts');
    expect(sessionLogWorkflow).toContain('apps/kite-cli/src/session-logger/**');
    expect(sessionLogWorkflow).toContain('apps/kite-cli/src/config/paths.ts');
  });
});
