...
describe('loadAgentConfig', () => {
  ...
  test('keeps model-route input source-owned and JSON-Schema representable', () => {
    expect(configSchema.parse({ model: 'ollama:qwen2.5-coder:7b' }).model).toEqual({
      default: { provider: 'ollama', name: 'qwen2.5-coder:7b' },
    });
    expect(configSchema.parse({ model: { default: 'ollama:qwen2.5-coder:7b' } }).model).toEqual({
      default: { provider: 'ollama', name: 'qwen2.5-coder:7b' },
    });
    expect(
      configSchema.parse({ model: { default: { provider: 'ollama', name: 'object-local' } } })
        .model,
    ).toEqual({ default: { provider: 'ollama', name: 'object-local' } });

    const inputSchema = z.toJSONSchema(configSchema, { io: 'input' });
    expect(JSON.stringify(inputSchema)).toContain('model');
    expect(JSON.stringify(inputSchema)).toContain('anyOf');
  });
