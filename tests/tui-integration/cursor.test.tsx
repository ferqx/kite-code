...
describe('TUI E2E — Cursor Behavior', () => {
  ...
  test(
    'slash suggestion: dropdown appears and ghost text is contiguous',
    async () => {
  ...
  test(
    'left arrow + type: char inserted at cursor position, not at end',
    async () => {
      await clearInput(tui);
  ...
  test(
    'right arrow + type: char inserted at cursor position',
    async () => {
      await clearInput(tui);
  ...
  test(
    'rapid nav + type: cursor stays correct through multiple operations',
    async () => {
      await clearInput(tui);
  ...
  test(
    'history navigation (up arrow) loads previous message into input',
    async () => {
  ...
  test(
    'history: up then down past end clears input',
    async () => {
...
describe('TUI E2E — No duplicate input when output has content', () => {
  const dupPlan = new ResponsePlan([
    {
      ...
      responses: [
        text(
          'Here is a response with some text to fill up the output area so we can verify the input line does not duplicate.',
        ),
      ],
    },
  ]);
  ...
  test(
    'typing after agent response: no duplicate ❯ below StatsLine',
    async () => {
