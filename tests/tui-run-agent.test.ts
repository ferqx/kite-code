...
function baseParams(overrides?: Partial<BaseTuiParams>): BaseTuiParams {
  return {
    ...
    shellExecutor: async (i: any) => ({ ok: true, command: i.command, exitCode: 0, stdout: "", stderr: "" }),
    signal: new AbortController().signal,
    thinkingLevel: "max",
    skills: [{ name: "tdd", description: "TDD workflow", source: "project" as const, origin: ".openpx" as const }],
    skillOptions: {
...
describe("buildRunAgentParams", () => {
  ...
  test("includes skills and skillOptions", () => {
    const skills = [{ name: "tdd", description: "TDD", source: "project" as const, origin: ".openpx" as const }];
    const skillOptions = { projectOpenpxSkillsDir: "/p", projectAgentsSkillsDir: "/a", userOpenpxSkillsDir: "/u", userAgentsSkillsDir: "/g" };
    const params = buildRunAgentParams({
