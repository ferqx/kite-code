...
function healthyHandshake(
  ...
  return {
    ...
    serverVersion: value.serverVersion ?? 'service-1',
...
describe('Kite Local Runtime lifecycle manager', () => {
  ...
  test('healthy instance rejects an expected build mismatch without spawning', async () => {
  ...
  test('healthy instance rejects a server-owned identity mismatch without spawning', async () => {
    const harness = createHarness(descriptor());
    harness.handshake = healthyHandshake({ serverVersion: 'different-service-version' });
    const result = await manager(harness).ensure({ requestId: 'server-version-mismatch' });
