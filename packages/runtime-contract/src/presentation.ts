...
export interface PlanCompletionEvidence {
  ...
  skipped: readonly { stepId: string; reasonCode: string }[];
  unresolved: readonly { kind: 'failure' | 'approval'; referenceId: string }[];
}
...
/** Runtime presentation boundary for Subagent lifecycle notifications. */
export type SubAgentEventSink = (
  event:
    ...
    | { type: 'cache_metrics'; data: SubAgentCacheMetricsPayload },
) => void;

/** Client-safe compaction progress; concrete compaction policy remains Builtin-owned. */
...
/** Read-only presentation snapshot; it carries no Context selection authority. */
...
/** Client-safe metadata projected from a Builtin-compiled Skill workflow. */
