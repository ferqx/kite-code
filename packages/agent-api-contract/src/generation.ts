...
const schemaRegistry = Object.freeze([
  ...
  ['AgentApiLogItem', 'log-item', agentApiLogItemSchema],
  ['AgentApiLogPage', 'log-page', agentApiLogPageSchema],
  ['AgentApiModelContext', 'model-context', agentApiModelContextSchema],
  ...
  return {
    ...
    '/v1/sessions/{session_id}/logs': {
      ...
      get: operation({
        id: 'listAgentApiLogs',
        tag: 'Logs',
        summary: 'Read a bounded durable client-safe diagnostic log page',
        security: authenticated,
        parameters: [afterSequenceParameter(), limitParameter(), cursorParameter()],
        success: { 200: jsonResponse('AgentApiLogPage', 'Diagnostic log page') },
        errors: [400, 401, 403, 404, 406, 409, 429, 503],
      }),
    },
    '/v1/sessions/{session_id}/model-invocations/{invocation_id}/context': {
      parameters: [sessionPath, pathParameter('invocation_id', 'Model invocation identity')],
      get: operation({
        id: 'getAgentApiModelContext',
        tag: 'Diagnostics',
        summary: 'Read one bounded Browser-only Model Context snapshot',
        security: browser,
        success: { 200: jsonResponse('AgentApiModelContext', 'Model Context snapshot') },
        errors: [400, 401, 403, 404, 406, 429, 503],
      }),
    },
