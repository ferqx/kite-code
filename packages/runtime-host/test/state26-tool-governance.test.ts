...
import {
  ...
  type RuntimeHostState26ToolGovernanceAuthorizationInputV1,
...
function context(): RuntimeHostState26ToolGovernanceAuthorizationInputV1['context'] {
  return {
    phase: 'building',
...
function input(
  ...
  overrides: Partial<RuntimeHostState26ToolGovernanceAuthorizationInputV1> = {},
...
function bridge(verifier: (value: ClassifiedInvocationV1) => boolean = () => true) {
  return createRuntimeHostState26ToolGovernanceV1({
    verifyClassifiedIdentity: verifier,
  });
}

describe('Runtime Host State26 tool governance bridge', () => {
  ...
  test('uses only an exact persisted State26 same-command grant', () => {
