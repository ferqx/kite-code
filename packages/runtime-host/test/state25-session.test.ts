...
describe('Runtime Host State25 session', () => {
  test('commits before publishing state and makes duplicate replay a no-write', () => {
    const f = fixture();
    const session = createRuntimeHostState25SessionV1(f.input);
    f.failCommit = true;
  ...
  test('fails closed on invalid facts, admission, and invariant input without a write', () => {
    ...
    const invalidSession = createRuntimeHostState25SessionV1({
      ...invalidClock.input,
      clock: () => 'not-a-state25-time',
    });
    expect(() => invalidSession.processEvent(message('invalid-time'))).toThrow(/timestamp/u);
    expect(invalidClock.writes).toHaveLength(0);

    const rejected = fixture();
    const admitted = createRuntimeHostState25SessionV1({
      ...rejected.input,
  ...
  test('retains a failed-restore State25 hard block until the runner durably aborts it', () => {
