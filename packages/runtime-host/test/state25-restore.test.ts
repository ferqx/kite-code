...
function restore(
  ...
  extra: Partial<
    Pick<Parameters<typeof restoreRuntimeHostState25SessionV1>[0], 'validateRestoredState'>
  > = {},
) {
  return restoreRuntimeHostState25SessionV1({
...
describe('Runtime Host State25 restore', () => {
  test('creates only the exact fresh State25 format when storage is empty', () => {
    const result = restore(services());
    expect(result.source).toBe('fresh');
    expect(result.state.schemaVersion).toBe(25);
    expect(result.state.formatEpoch).toBe('kite-runtime-2026-08-18');
