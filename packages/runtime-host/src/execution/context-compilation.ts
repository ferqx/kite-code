...
export interface RuntimeHostContextCompilationPort {
  compile(request: RuntimeHostContextCompilationRequest): Promise<CompiledContext>;
}

/** Host mechanism only: collect registered pure projections, validate bounds, then call Builtin. */
export function createRuntimeHostContextCompilationPort(
  registry: RuntimeModuleRegistry,
  compiler: ContextCompilerPort,
): RuntimeHostContextCompilationPort {
  return createRuntimeHostContextCompilationPortFromSnapshot(
    registry,
    compiler,
    registry.snapshot(),
  );
...
export function createRuntimeHostContextCompilationPortFromSnapshot(
  registry: RuntimeModuleRegistry,
  compiler: ContextCompilerPort,
  ...
  return Object.freeze({
    async compile(request: RuntimeHostContextCompilationRequest): Promise<CompiledContext> {
      ...
      validateCompiledContext(compiled, candidates, request.tokenBudget);
      return compiled;
    },
  });
}

function validateCandidates(candidates: readonly ContextFragmentCandidate[]): void {
  ...
  for (const candidate of candidates) {
    if (
      ...
      !isRuntimeJsonValue(candidate.content)
    ) {
...
function validateCompiledContext(
  compiled: CompiledContext,
  candidates: readonly ContextFragmentCandidate[],
  tokenBudget: number,
): void {
  ...
  if (tokens > tokenBudget || !isRuntimeJsonValue(compiled.payload)) {
    throw new Error('Context Compiler returned an invalid or over-budget payload.');
  }
}

function isRuntimeJsonValue(value: unknown, seen = new Set<object>()): boolean {
  ...
  const valid = Array.isArray(value)
    ...
    : Object.getPrototypeOf(value) === Object.prototype &&
      Object.values(value).every((item) => isRuntimeJsonValue(item, seen));
  seen.delete(value);
  return valid;
}
