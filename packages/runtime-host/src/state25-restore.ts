...
export type State25RuntimeRestoreSourceV1 = 'fresh' | 'restored' | 'incompatible' | 'corrupted';

interface State25RuntimeRestoreInputBaseV1 {
  readonly sessions: SessionStore<KernelEvent, AgentState>;
  readonly sessionId: string;
  readonly userId: string;
  readonly workspace: string;
  /** Host-allocated identity for a fresh State25 session. */
   ...
   * failure as corrupted State25 data and never interprets the evidence.
   */
  readonly validateRestoredState?: (state: Readonly<AgentState>) => void;
}

export type State25RuntimeRestoreInputV1 =
  | (State25RuntimeRestoreInputBaseV1 & {
...
export interface State25RuntimeRestoreResultV1 {
  readonly state: AgentState;
  readonly restoreBoundary: RuntimeRestoreBoundaryV1;
  readonly source: State25RuntimeRestoreSourceV1;
}
...
function replayCurrentState25Tail(
  state: AgentState,
  ...
  for (const entry of tail) {
    ...
    if (entry.event.type === 'user.message_appended' && current.activeTaskId === null) {
      throw new Error(
        `Runtime event ${entry.id} requires the State25 snapshot that owns its Host-allocated Task identity.`,
      );
    }
...
function freshState25(input: State25RuntimeRestoreInputV1): AgentState {
  const common = {
...
/** Restore only State25 / Store4 / the current RMV1 epoch from injected Host services. */
export function restoreRuntimeHostState25SessionV1(
  input: State25RuntimeRestoreInputV1,
): State25RuntimeRestoreResultV1 {
  const freshState = freshState25(input);
  ...
  try {
    if (!isCurrentAgentStateSnapshot(snapshotRecord.state)) {
      throw new Error('Runtime snapshot is not an exact current State25 record.');
    }
    ...
    if (
      ...
      throw new Error('Runtime snapshot metadata does not match the State25 payload.');
    }
    state = replayCurrentState25Tail(
      state,
