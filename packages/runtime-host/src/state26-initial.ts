 ...
 */
type MutableState26<T> = T extends (...args: never[]) => unknown
  ? T
  : T extends readonly unknown[]
    ? T
    : T extends object
      ? { -readonly [Key in keyof T]: MutableState26<T[Key]> }
      : T;

export type RuntimeState = MutableState26<AgentState>;
export type TaskState = MutableState26<AgentTaskState>;
export type ToolCallRecord = MutableState26<AgentToolCallState>;
export type ToolCallStatus = AgentToolCallState['status'];
export type ToolResultMeta = AgentToolResultMeta;

export interface RuntimeHostState26InitialStateInputV1 {
 ...
 */
export function createRuntimeHostState26InitialStateV1(
  input: RuntimeHostState26InitialStateInputV1,
): RuntimeState {
