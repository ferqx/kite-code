...
import { modelArtifactRoot, userKiteCodeDirV1 } from './artifact-paths';
...
export function allPrivateArtifactEvidenceRootsV1(): readonly string[] {
  const installation = userKiteCodeDirV1();
  return Object.freeze([
    join(installation, 'capability-artifacts'),
    join(installation, 'filesystem-preimages'),
    join(installation, 'sandbox-preparations'),
    join(installation, 'subagent-tasks'),
    join(installation, 'subagent-lifecycles'),
    join(installation, 'subagent-continuations'),
  ]);
}
...
export function loadOrCreateModelArtifactIntegrityKeyV1(
  ...
  const keyPath = resolve(options.keyPath ?? join(userKiteCodeDirV1(), KEY_FILE_NAME));
  const artifactPaths = [
