...
- Child-owned tool events may carry the exact bounded `presentationOwner { subagentId, parentToolCallId }` on queued and terminal lifecycle facts. Preserving it on terminal-only replay keeps hidden child failures inside their parent task without parsing tool IDs or relying on event order.
- `subagent.started.parentToolCallId`, when present, is the exact parent task-tool identity used only for presentation ownership; older or detached records may omit it.
